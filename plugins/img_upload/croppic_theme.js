...
(function (window, document) {
	...
	Croppic = function (id, options,element) {

		var that = this;
		that.id = id;
		that.element = element;
		that.tooltip = element['tooltip'];
		that.obj = $(id);
		that.outputDiv = that.obj;
		that.pos = element['pos'];
		// DEFAULT OPTIONS
		that.options = {
			uploadUrl:'',
			uploadData:{},
	...
	Croppic.prototype = {
		...
		init: function () {
			var that = this;
			// if (that.objH == 208) {
			// 	that.objW = that.objW + 22;
			// 	that.objH = that.objH + 35;
			// }
			if(that.element['type']=='background'){
				...
				if(pos){
					that.objW = that.obj.width();
					that.objH = that.obj.height();
					//$(that.id).css({"width": that.objW, "height": that.objH});
				}else{
					that.objW = that.obj.find('img').width();
					that.objH = that.obj.find('img').height();
					//$(that.id).find('img').css({"width": that.objW, "height": that.objH});
				}
			}

			if(that.objH > (that.windowH - 100)){
				that.zoomClz = true;
			}
			// reset rotation
			that.actualRotation = 0;
			
			if( isEmpty(that.defaultImg)){ that.defaultImg = that.obj.find('img'); }
			
			that.createImgUploadControls();
			
			if( isEmpty(that.options.loadPicture)){				
		...
		createImgUploadControls: function(){
			...
			if( isEmpty(that.croppedImg)){ cropControlRemoveCroppedImage=''; }
			if( !isEmpty(that.options.loadPicture)){ cropControlUpload='';}

			if(that.element['type']=='background'){
				var direc = 'data-direction="'+that.element['direction']+'"';
			}else{
				var direc = '';
			}

			var html =  '<div class="cropControls cropControlsUpload editable-icon-upload" '+direc+' data-ref="'+that.id+'"> ' + cropControlUpload + cropControlRemoveCroppedImage + that.tooltip +' </div>';
			if(that.element['element']=='.verticle_img' || that.element['element']=='.Horizontal_img'){
				html += '<div class="gallery-icon" data-ref="'+that.id+'"><span class="tooltip">Click here to choose an image from the Cox Image Gallery. It contains royalty free images you can use to customize your splash page.</span></div>';
			}
			if(that.pos){
				that.outputDiv.after(html);
			}else{
				that.outputDiv.append(html);
			}
			
			//that.cropControlsUpload = that.outputDiv.find('.cropControlsUpload');
			that.cropControlsUpload = $("[data-ref='"+that.id+"'].cropControlsUpload");
			
			if(that.options.customUploadButtonId ===''){ that.imgUploadControl = $("[data-ref='"+that.id+"'].cropControlsUpload"); }
			else{	that.imgUploadControl = $('#'+that.options.customUploadButtonId); that.imgUploadControl.show();	}

			if( !isEmpty(that.croppedImg)){
		...
		bindImgUploadControl: function(){
            ...
            var formHtml = '<form class="' + that.id.match(/[a-zA-Z0-9-_]+/g) + '_imgUploadForm" style="display: none;">  <input type="file" name="img" id="' + that.id + '_imgUploadField">  </form>';
			if(that.pos){
				that.outputDiv.after(formHtml);
			}else{
			that.outputDiv.append(formHtml);
			}
			that.form = $('.'+that.id.match(/[a-zA-Z0-9-_]+/g)+'_imgUploadForm');
			...
			that.imgUploadControl.on('click',function(){ 
				if(that.element['alt-txt']=='enabled'){ $(that.element['element']).parent().find('.alttxt').hide();$(that.element['element']).parent().find('img').show(); }
				$('#trigClickPos').trigger('click');
				if (fileUploadId === "") {
				    ...
				    if (typeof FileReader == "undefined"){
						...
						reader.onload = function (e) {
							...
							image.onload = function(){
								...
								if( !isEmpty(that.croppedImg)){ that.croppedImg.remove(); }
		...
		afterUpload: function(data){
            ...
            if (response.status == 'success') {
            	...
            	try {
    				var uptag = response.uptag;
    				
    				document.getElementById(uptag).value = response.uppath;
            	}
            	catch(err) {
            	...
            	try {
    				var hid_img = response.hi_tag;
    				
    				var image_x = document.getElementById(hid_img);
		...
		createModal: function(){
			...
			var modalHTML =  '<div id="croppicModal" class="'+clz+'">'+'<div id="croppicModalObj" style="width:'+ that.objW +'px; height:'+ that.objH +'px; margin:0 auto; margin-top:'+ marginTop +'px; position: relative;"> </div>'+'</div>';
		...
		initCropper: function(){
			...
			that.img.wrap('<div class="cropImgWrapper" style="overflow:hidden; z-index:1;border: 1px dashed #736f6f; position:absolute; width:'+that.objW+'px; height:'+that.objH+'px;"></div>');
		...
		afterCrop: function (data) {
            ...
            if (response.status == 'success') {
				...
				var urlforval = response.url;
				var filenameforval = urlforval.substring(urlforval.lastIndexOf('/')+1);
				
				var itag = response.itag;
				
				//document.getElementById(itag).value = filenameforval;
				...
				if(that.element['type']=='background'){
					$("+that.id+").append('<div class="loader bubblingG themeLoader"><span id="bubblingG_1"></span><span id="bubblingG_2"></span><span id="bubblingG_3"></span></div>');
						that.obj.css('background', 'url(' + response.url + ')').attr('data-name', filenameforval).load(function(){
							$('.themeLoader').remove();
						});
				}else{
					if(that.pos){
						$("[data-ref='"+that.id+"']").append('<div class="loader bubblingG themeLoader"><span id="bubblingG_1"></span><span id="bubblingG_2"></span><span id="bubblingG_3"></span></div>');
						that.obj.attr("src",response.url ).attr('data-name', filenameforval).load(function(){
							$('.themeLoader').remove();
						});

					}else{
						
						that.obj.find('img').attr("src",response.url ).after('<div class="loader bubblingG themeLoader"><span id="bubblingG_1"></span><span id="bubblingG_2"></span><span id="bubblingG_3"></span></div>').attr('data-name', filenameforval).load(function(){
							$('.themeLoader').remove();
						});
						}
				}
		...
		destroy:function(){
			var that = this;
			if(that.options.modal && !isEmpty(that.modal) ){ that.destroyModal(); }
			if(that.options.imgEyecandy && !isEmpty(that.imgEyecandy) ){  that.destroyEyecandy(); }
			if( !isEmpty( that.cropControlsUpload ) ){  that.cropControlsUpload.remove(); }
			if( !isEmpty( that.cropControlsCrop ) ){   that.cropControlsCrop.remove(); }
			if( !isEmpty( that.loader ) ){   that.loader.remove(); }
			if( !isEmpty( that.form ) ){   that.form.remove(); }
			that.obj.html('');
		},
