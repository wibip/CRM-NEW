...
if (isset($_POST['provisioning_submit'])) {
    if ($_SESSION['FORM_SECRET'] == $_POST['provisioning_form_secret']) {
        ...
        $result = $provisioning->create($data_bulk);

        if ($result) {
            $_SESSION['msg1'] = "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button><strong>" . $message_functions->showMessage('service_type_create_success', '2000') . "</strong></div>";
        } else {
            $_SESSION['msg1'] = "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button><strong>" . $message_functions->showMessage('service_type_create_fail', '2000') . "</strong></div>";
        }
    }
}
