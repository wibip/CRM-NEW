...
@media screen and (max-width: 979px){
  #search_word, #search_mac, #search_word, #search_mac {
...
#device_form .controls, #edit_customer_form .controls, #passcode_form .controls, #device_pass_form .controls{
    width: auto;
    margin: auto;
}
...
$query_service_profile = "SELECT `product_code` AS f FROM `exp_products_distributor` WHERE distributor_code='$user_distributor' AND `network_type` = 'VT-DEFAULT'";
$service_profile_product=$db->getValueAsf($query_service_profile);
...
<?php 
//echo "SELECT `property_type` AS f FROM `mdu_mno_distributor` WHERE `distributor_code`='$user_distributor'"; exit();
...
function syncCustomerNew($Response2,$property_id,$acc_data,$mac_format=''){
  ...
  //print_r('asdfasddf');
      ...
      //$bulk_profile_datasession;
        ...
        for($k=0;$k<$single_profile_data1;$k++){
            ...
            $ID = mysql_real_escape_string($single_profile_data_ar['Session-Id']);
              ...
              //$Account_State=mysql_real_escape_string($single_profile_data_ar['Account-State']);
                ...
                $Nas_Type=mysql_real_escape_string($single_profile_data_ar['Nas-Type']);
                if (strlen($Gw_Type)< 1) {
                   $Gw_Type=$Nas_Type;
                }
        ...
        $CLIENTID=$db->macFormat($CLIENTID,$mac_format);
                    $newarray1=array("Mac"=> $CLIENTID,
                                "F-Name" => $fname,
                                "L-Name" => $lname,
                                "Email"=> $Email,
              ...
              $testmac=$single_profile_data_ar['Session-MAC'];
              $testmac=$db->macFormat($testmac,$mac_format);

                $arraytest=array("Mac"=> $testmac,//$single_profile_data_ar['Session-MAC'],
                                  "Realm"=> $single_profile_data_ar['Access-Group'],
                                  "Nas-type"=> $Gw_Type,//$single_profile_data_ar['Access-Group'],
...
$find_customers=mysql_query("SELECT d.`customer_id`  AS f FROM `mdu_customer_devices` d JOIN `mdu_vetenant` v ON d.`customer_id` = v.`customer_id` WHERE d.`description` LIKE '%$search_mac%' and  v.`property_id`='$property_id'");
...
if(isset($_POST['device_pass_submit'])){ 
	$mg_customer_id = $_POST["mg_customer_id"];
	$property_id=$mg_property_id;
  $passcode_key = trim($_POST["passcode_key"]);
  

  $cust_name_q = "SELECT `username` AS f FROM `mdu_vetenant` WHERE `customer_id`='$mg_customer_id'";
  $cust_uname=$db->getValueAsf($cust_name_q);;
  ...
  if (strlen($network_profile) > 0) {
    ...
    $nf = null;
				...
				$response = $nf->modifyAccDpsk($cust_uname,$passcode_key);
  ...
  $record_found=1;
	...
	dpsk_key = '$passcode_key'
	WHERE username = '$cust_uname' ";

	$ex= mysql_query($query01);
	if($ex){
		$msg_edit="<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>×</button><strong>Device password change success. </strong></div>";

/*    $msg_warning="<div class='alert alert-danger'><strong>Connected devices will remain connected to Wi-Fi. After you disconnect a device, use your new password to reconnect to Wi-Fi.</strong></div>";*/

		
	}else{
		$msg_edit="<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button><strong>Device password change failed. </strong></div>";

	}
// }else{
// 	$_SESSION['msg_edit']= "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button><strong>".$message_functions->showMessage('vtenant_dpsk_failed','2001')."</strong></div>";
// }
 ...
 */
  ...
  $acc_response = $nf->checkMasterAccount($mg_uname);
  $account_status = json_decode($acc_response,true);
  $status_code=$account_status['status_code'];
  ...
  if ($status_code=='200') {
   ...
   $customerOb -> syncCustomerDevices($device_response,$mg_uname,uniqid($mg_uname));
    ...
    $del_vlanid = $db->getValueAsf("SELECT vlan_id as f FROM mdu_vetenant WHERE `customer_id`='$mg_customer_id'");
    $del_prop = $db->getValueAsf("SELECT property_id as f FROM mdu_vetenant WHERE `customer_id`='$mg_customer_id'");

    $vlanOb->addDeleteVlanID($del_prop,$del_vlanid);
    $query0 = "DELETE FROM mdu_vetenant WHERE `customer_id`='$mg_customer_id'";
    $query_de = "DELETE FROM mdu_customer_devices WHERE `customer_id`='$mg_customer_id'";
    //$query0 = "DELETE FROM mdu_vetenant WHERE `username`='$mg_uname'";
     $ex0 = mysql_query($query0);
     $ex1 = mysql_query($query_de);

    $_SESSION['msg']="<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>X</button><strong>Account doesnot exits </strong></div>";
    ...
    //if (mysql_num_rows($chek_mail) == 0) {//mail
      ...
      if ($status_code == 200 || $status == 200) {
        ...
        if ($password != "") {
          ...
          $distributor_code=$db->getValueAsf("SELECT distributor_code AS f FROM mdu_distributor_organizations WHERE property_id='$property_id'");
          $property_name=$db->getValueAsf("SELECT distributor_name AS f FROM exp_mno_distributor WHERE distributor_code='$distributor_code'");
        ...
        if($get_property_type=='VTENANT'){
          ...
          $varss = array(
            ...
            $subject_all = strtr($subject, $varss);
...
$mail_sent=$mail_obj->sendEmail($from,$to,$subject_all,$message_full,'',$title);
...
$querynew="SELECT u.`ssid`,u.`network_id`,d.`zone_id` FROM `exp_locations_ssid_vtenant` u JOIN `exp_mno_distributor` d ON u.`distributor`=d.`distributor_code` WHERE  d.`distributor_code` = '$user_distributor' AND `wlan_name` LIKE '%vtenant%' ";
$ex3 = mysql_query($querynew);
$data_zone=mysql_fetch_assoc($ex3);
$zoneId=$data_zone['zone_id'];
$wlanId=$data_zone['network_id'];
$v_ssid=trim($data_zone['ssid']);
                   ...
                   <div class=\"modal-dialog\">
                     ...
                     <div class=\"modal-content\">
                       <div class=\"modal-body\">
                         <img style=\"width: 100%;\" src=\"img/Modal_Image_dpsk.png\">

                         <div class=\"sub-body\">

                         <div class=\"wifi-body\">
                        <p>Disconnect from current SSID and Reconnect to the \"".$v_ssid."\" </p> 
                         </div>
                         <div class=\"etheret-body\">
                         <p>Disconnect the cable and reconnect the cable </p>  
                         </div>
  ...
  $mail=$prof_mail['username'];
    ...
    if($cecrot_val==$_SESSION['PROF_RAND']){
      ...
      if ($profile_type=='DEFAULT') {
          $device_response=$nf->removeQOSProfile($mail,$new_profile);
         }
      else{
          $device_response=$nf->modifyQOSProfile($mail,$mg_qos_code);
      }
          parse_str($device_response);
          //$status_code = 200;
          if($status_code == 200 || $status == 200){
        $status_code ="" ;
        $status ="";
        $Response = $nf->getSessions($mail,'AAA-User-Name');
        $bulk_profile_data = json_decode($Response,true);
        
        $datasession=$bulk_profile_data['Description'];
        foreach ($datasession as $key => $value) {
          $ses_token=$value['Session-Token'];
          $device_response=$nf->disconnectDeviceSessions($ses_token,$mail);
        }
        if ($profile_type=='DEFAULT') {
          
        $upprof_main = "UPDATE `mdu_vetenant` SET service_profile = '$new_profile',profile_type = '$profile_type',qos_override = '' WHERE customer_id = '$mg_customer_id'";
        $prof_chang_ex = mysql_query($upprof_main);
      }
        else{
          $upprof_main = "UPDATE `mdu_vetenant` SET qos_override = '$new_profile',profile_type = '$profile_type' WHERE customer_id = '$mg_customer_id'";
        $prof_chang_ex = mysql_query($upprof_main);
        }
        ...
        if ($prof_chang_q) {
          ...
          $msg_edit = "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>X</button>
          <strong>".$message_functions->showMessage('qos_pro_update_success','2002')."</strong></div>";

          
          //Service Activation Report
          if($profile_type == 'default'){
            $service_id='GENERIC_PROFILE';
          }
          if($profile_type == 'probation'){
            $service_id='PROBATION_PROFILE';
          }
          if($profile_type == 'premium'){
            $service_id='PREMIUM_PROFILE';
          }
         
         $activation_type ='Activated';
         $reference=$mail;
         $db->changeFeature(new FeatureChange($service_id, $activation_type, $user_distributor, $user_type, $reference));

        }

          } else{
              $msg_edit= "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>X</button><strong>".$message_functions->showMessage('qos_pro_update_failed','2002')."</strong></div>";
          }
...
///////////////////////////////////////////////GET Customer INFO//////////////////////////////////////////////
 $qq_156 = "SELECT c.duration, c.profile_type, c.service_profile, c.vlan_id, c.`first_name`,c.`last_name`,c.`email`,c.username,c.`password`,c.`dpsk_key`,c.room_apt_no,c.`property_id`,o.`org_name`,c.`question_id`,c.`answer`,c.company_name,c.address,c.city,c.state,c.postal_code,c.country,c.phone,o.property_type
...
if(mysql_num_rows($get_cust_info)>0){
   ...
   $get_qos_override =$rowc['qos_override'];
...
if(strlen($search_word)>0){
  ...
  if ($string_type=='mac') {
    ...
    $Response = $nf->getSessions($mac,'Session-MAC');
    $newjsonvalue=syncCustomerNew($Response,$property_id,$accdata,$mac_format);
    # code...
  }
  elseif($string_type=='two'){
    //echo "test";
    $searchword=explode(' ', $search_word);
    $first_name=$searchword['0'];
    $last_name=$searchword['1'];
    //print_r($search_word);
    $user_prop_list = $db->get_property($user_distributor);
    foreach($user_prop_list as $row_prop){
    ...
    $newarray=syncCustomerNew($Response,$property_id,$accdata,$mac_format);
    $newarray1=$newarray[0];

    $res_array=array();
    foreach ($newarray1 as $value){
          if(($value['F-Name']==$first_name) AND ($value['L-Name']==$last_name)){
                array_push($res_array, $value);
      }
    }
      
    $newjsonvalue=array($res_array,$newarray[1]);
  ...
  elseif ($string_type=='name') {
   ...
   /* $Response1 = $nf->getSessions($search_word,'PAS-First-Name');
    ...
    $newjsonvalue3=syncCustomerNew($Response3,$property_id,$accdata,$mac_format);
...
elseif (strlen($search_mac)>0) {
  $Response = $nf->getSessions($search_mac,'Session-MAC');
  $newjsonvalue=syncCustomerNew($Response,$property_id,$accdata,$mac_format);
  }
else{
  if ($property='ALL'){
              ...
              <div class="widget-content" id="now">
                ...
                <div class="tabbable">
                  <ul class="nav nav-tabs newTabs">
                    <li <?php if($active_tab=="account"){echo "class=\"active\"";}?>><a href="#account" data-toggle="tab">Manage Residents</a></li>
...
//alert('aa');
        ...
        var method='vtenant';
        var init_auth_data = {realm:realm,mac:mac,method:method};
         ...
         $('#city').bind("cut copy paste",function(e) {
                                ...
                                <div class="control-group">
                                     ...
                                     <label class="" for="radiobtns">Mobile Number</label>
                                         ...
                                         <input class="span4 form-control mobile1_vali" id="phone" placeholder="xxx-xxx-xxxx" name="phone" type="text" maxlength="12" value="<?php echo $mg_phone; ?>" oninvalid="setCustomValidity('Invalid mobile number format')" oninput="setCustomValidity('')" onfocus="setCustomValidity('')" style="display: block; width:70%;">
         ...
         function customer_submitenable(){
           document.getElementById("customer_submit").disabled = false;
             ...
             <div class="">
               ...
               <h3><strong>Tenant QoS Profile</strong></h3>
               </div>
                 ...
                 <div class="widget-content">
                   ...
                   <?php
                    ...
                    /*  $query_results_profile=mysql_query($query_service_profile);
                      ...
                      $resultArr = mysql_fetch_array($query_results_profile);
...
*/                         echo " <div class='txt_qos_new'>Default Product [".$service_profile_productn."]</div>";
         ...
         <?php            
                    $get_dist_profile = "SELECT d.`id`,d.`qos_code`,q.`qos_name`,q.`network_type`,d.`qos_id` as qos FROM exp_qos_distributor d LEFT JOIN exp_qos q ON d.`qos_id`=q.`qos_id`
                      ...
                      $row_count = mysql_num_rows($query_results_profile);
                    ...
                    $flag = 0;
                      ...
                      <?php }
                                ...
                                if($flag == 1){
                                  ...
                                  echo '<div style="margin-bottom: 5px;" id="q_override"><b>Override QoS Profile</b><img data-toggle="tooltip" title="Use the Search Tenant field to search for existing tenants that have contacted you for assistance with their account. This could include updating personal information, adding or removing devices, upgrading account to a premium service, resetting their account password and also permanently deleting an account." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></div>';
            ...
            <h3><strong>Add New Unique Wi-Fi password</strong></h3>  
<br>
                         <form id="device_pass_form" name="device_pass_form" action="manage_tenant.php?tab=account" method="post"  class="form-horizontal" enctype="multipart/form-data" autocomplete="off">
								...
								?>
									...
									<fieldset>
                                     ...
                                     <label class="" for="radiobtns">Unique Wi-Fi password<img data-toggle="tooltip" class="tooltips" title="Connected devices will remain connected to Wi-Fi. After you disconnect a device, use your new password to reconnect to Wi-Fi." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></label>

                                     <div class="pass_de"><i toggle="#passcode_key_de" style="display:inline !important;margin-left: -25px; " class="paas_toogle btn-icon-only icon-eye-open com_toggle-password_de" id="de_pass"></i>

 													
                                     
					<input class="span4 form-control " id="passcode_key_de" style="width:70%;" placeholder="******" name="passcode_key" type="password"  value="<?php echo $mg_passcode_key; ?>" >
               </div>
          <p>Once a  device has been registered via the self-care portal or by using the New Account Wi-Fi netowrk registration process, the resident uses this unique Wi-Fi password to logon to the Resident Wi-Fi Network. The password must be a at least 8 characters long.</p>
														</div>
														<!-- /controls -->
													</div>
													<!-- /control-group -->
...
if (input.attr("type") == "password") { 
  ...
  $('#passcode_key_de').attr("type", "password");
													...
													<div class="form-actions" class="form-actions" style="margin-top: -20px; background-color: #fff;    border-bottom: 1.3px dotted #9c9b9b;padding-left: 0px; width: 350px; ">
													
												<br>
														<button type="submit" name="device_pass_submit" id="device_pass_submit" style="display: inline-block;" class="btn btn-primary" disabled>Update </button>&nbsp;<!-- <strong><font color="#FF0000">*</font><font size="-2"> Required</font></strong>-->
...
<div style="overflow-x:auto">
                            ...
                            <table class="table table-striped table-bordered divice_con">
														<thead>
															<tr>
																<th>#</th>
																<th>NICKNAME</th>
...
$get_user_name=mysql_query("SELECT username AS f FROM `mdu_vetenant` WHERE `customer_id`='$mg_customer_id' LIMIT 1");
                   ...
                   $key_query_d="SELECT d.id,d.nick_name,d.email_address,d.dpsk_key ,IF(d.description IS NULL ,t2.mac,d.description) AS description ,d.mac_address,IF(t2.mac IS NULL,'d','b') AS s_type ,
                         ...
                         UNION ALL
                         SELECT d.id, d.nick_name,d.email_address,d.dpsk_key,IF(d.description IS NULL ,t2.mac,d.description) AS description,d.mac_address,'s' AS s_type ,
                         t2.start_time, t2.account_state,d.create_user,
                         t2.session_id
                    ...
                    $key_query_d = "SELECT  id,`nick_name`,`email_address`,`description`,`mac_address`,`key_query_d` 
                    FROM `mdu_customer_devices` WHERE `user_name`='$cust_user_name'";
            

                   }    
                  
                                   $query_results_d=mysql_query($key_query_d);
								while($row=mysql_fetch_array($query_results_d)){
                         ...
                         if(mysql_num_rows($ex_mdu)>0){
                           ...
                           $session_active = 'NO';
									...
									if($_GET['ed_device_id']==$id){
										...
										?>
							<tr>
<form id="device_edit_form<?php echo $id; ?>" name="device_edit_submit<?php echo $id; ?>" action="manage_tenant.php?tab=account" method="post"  class="form-horizontal" enctype="multipart/form-data" autocomplete="off" onsubmit="return edit_validateForm<?php echo $id; ?>()">
      <td><?php echo $no; ?></td>                   
<td><div class="form-group" id="td_nick_name<?php echo $id; ?>"> <input class="span2 form-controls nick_name" type='text' id="nick_name<?php echo $id; ?>" name='nick_name'   value="<?php echo $nick_name; ?>" >
...
<td><?php echo $db->macFormat($mac_relm[0],$mac_format); ?></td>
									...
									<?php //if($dpsk_enable){?>
                        <!-- <td> 
						<div class="form-group" id="td_dpsk<?php //echo $id; ?>"><input class="span4 form-controls" type='text' id="div_passcode_key<?php //echo $id; ?>" name='div_passcode_key' value="<?php //echo $dpsk_key; ?>"> 
						<small class=" help-block"  id="msg_dpsk<?php //echo $id; ?>"   style="display: none;"></small>
						...
						<button type="submit" style="border-radius: 0px;padding-bottom: 4px !important; padding-top: 4px !important; min-width: 0 ;" href="javascript:void();" id="device_edit_submit<?php echo $id; ?>" name="device_edit_submit" class="btn btn-primary">
																Edit</button></td>
                        <script type="text/javascript">
                        $(document).ready(function() {
                        $('#device_edit_submit<?php echo $id; ?>').easyconfirm({locale: {
                                title: 'Edit Device ',
                                text: 'Are you sure you want to Edit this device?&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',
                                button: ['Cancel',' Confirm'],
                                closeText: 'close'
                                 }});
...
function edit_validateForm<?php echo $id; ?>() {
	...
	// if($("#div_passcode_key<?php //echo $id; ?>" ).val()!=""){

	// 	var dpsk_value = $("#div_passcode_key<?php //echo $id; ?>" ).val();

	// 	var formData = {user_distributor: '<?php //echo $user_distributor ?>',div_passcode_key:dpsk_value,div_id: '<?php //echo $id ?>'};
    //                                                                         $.ajax({
	...
	// 	$("#msg_dpsk<?php //echo $id; ?>" ).text( dpsk_msg );
	// 	$("#td_dpsk<?php //echo $id; ?>").addClass("has-error");			
	// 	$("#msg_dpsk<?php //echo $id; ?>").css("display", "block");
	// }else{
	// 	$("#msg_dpsk<?php //echo $id; ?>" ).text( dpsk_msg );
	// 	$("#td_dpsk<?php //echo $id; ?>").removeClass("has-error");			
	// 	$("#msg_dpsk<?php //echo $id; ?>").css("display", "none");
	// }
 	return false;
	}
...
// 									<a  style="border-radius: 0px; background-color: white !important;color: black !important;" href="javascript:void();"  id="DR1_'.$id.'"  class=" ">
// 																<i class="btn-icon-only icon-pencil"></i></a></td>
																
// 																<script type="text/javascript">
// $(document).ready(function() {
// $(\'#DR1_'.$id.'\').easyconfirm({locale: {
// 		title: \'Edit Connected Device \',
// 		text: \'Are you sure you want to edit this connected device?&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\',
// 		button: [\'Cancel\',\' Confirm\'],
// 		closeText: \'close\'
// 	     }});
// 	$(\'#DR1_'.$id.'\').click(function() {
// 		window.location = "?s_token='.$secret.'&customer_id='.$mg_customer_id.'&search_id='.$search_id.'&ed_device_id='.$id.'"
// 	});
// 	});
// </script>
// 									';
...
echo '<td> '.$session_active.' </td>';
								...
								for($di=1; $di <= ($max_allowed_devices-$device_count); $di++){
									$no++;
					?>

<tr>
<form id="device_form<?php echo $di; ?>" name="device_form<?php echo $di; ?>" action="manage_tenant.php?tab=account" method="post"  class="form-horizontal" enctype="multipart/form-data" autocomplete="off" onsubmit="return validateForm<?php echo $di; ?>()">
<td><?php echo $no; ?></td>                          
<td><div class="form-group" id="td_nick_name<?php echo $di; ?>"> <input class="span2 form-controls nick_name" type='text' id="nick_name<?php echo $di; ?>" name='nick_name'   > 
...
<td><div class="form-group" id="td_mac<?php echo $di; ?>"> <input class="span2 form-controls" type='text' id="mac_address<?php echo $di; ?>" name='mac_address'  > 
<small class=" help-block"  id="msg_mac<?php echo $di; ?>"   style="display: none;"></small>
                        ...
                        <script type="text/javascript">
                        $(document).ready(function() {
                        $('#device_submit<?php echo $di; ?>').easyconfirm({locale: {
                                title: 'Add Device ',
...
function validateForm<?php echo $di; ?>() {
	...
	// if($("#div_passcode_key<?php //echo $di; ?>" ).val()!=""){

	// 	var dpsk_value = $("#div_passcode_key<?php //echo $di; ?>" ).val();

	// 	var formData = {user_distributor: '<?php //echo $user_distributor ?>',div_passcode_key:dpsk_value};
    //                                                                         $.ajax({
    //                                                                             url: "ajax/dpsk_val.php",
    //                                                                             type: "POST",
	// 																			async: false,
	...
	// 	$("#msg_dpsk<?php //echo $di; ?>" ).text( dpsk_msg );
	// 	$("#td_dpsk<?php //echo $di; ?>").addClass("has-error");			
	// 	$("#msg_dpsk<?php //echo $di; ?>").css("display", "block");
	// }else{
	// 	$("#msg_dpsk<?php //echo $di; ?>" ).text( dpsk_msg );
	// 	$("#td_dpsk<?php //echo $di; ?>").removeClass("has-error");			
	// 	$("#msg_dpsk<?php //echo $di; ?>").css("display", "none");
	// }
 	return false;
	}
                         ...
                         ?>
                           ...
                           <div class="se_field">
                             ...
                             <div class="control-group">
                                     ...
                                     <div class="controls form-group">
                                       <div class="columns" style="margin-bottom: 5px">
                                          <h3><strong style="padding-left: 3%;">Manually Add Tenant Devices</strong></h3>
                                       </div>
                                     </div>
                                   ...
                                   <div class="control-group" style=" margin-left: 10px;" >
                                     ...
                                     <label class="" for="radiobtns">MAC Address<sup><font color="#FF0000" >*</font></sup></label>
                                         ...
                                         <input class=" form-control"  id="mac_address" name="mac_address" maxlength="17" type="text" style="width:82%;"  oninvalid="setCustomValidity('Invalid MAC format')" oninput="setCustomValidity('')" onfocus="setCustomValidity('')"  title="Format - xx:xx:xx:xx:xx:xx" pattern="^[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}$" >
...
<?php } ?>
         ...
         <?php } else{ ?>      
                              ...
                              <h1 class="head"><span>
    Manage Residents <!-- <img data-toggle="tooltip" title="Use the Search Tenant field to search for existing tenants that have contacted you for assistance with their account. This could include updating personal information, adding or removing devices, upgrading account to a premium service, resetting their account password and also permanently deleting an account." src="layout/ATT/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;"> --></span>
</h1>
                           ...
                           <fieldset>
                                   ...
                                   <div class="control-group" style="display: none;">
                                     ...
                                     <div class="controls">
                                      ...
                                      <label class="" for="radiobtns">Limit Search to <img data-toggle="tooltip" title="You can search a tenant across all your properties or limit the search to a specific property." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></label>
                                   ...
                                   <div class="control-group middle-large">
                                     ...
                                     <div class="controls">
                                      ...
                                      <label class="" for="radiobtns">Search Residents <img data-toggle="tooltip" title="You can search for resident using full or partial First Name, Last Name, or Email or use the Search MAC field." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></label>
                                     ...
                                     <div class="controls">
                                      ...
                                      <label class="" style="margin-top: 10px" for="radiobtns">Search MAC <img data-toggle="tooltip" title="You can search for resident using full or partial MAC address of a device or use the Search Tenant field." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></label>
                                     ...
                                     <div class="controls">
                                      ...
                                      <button class="btn" style="margin-bottom: 10px" type="submit" name="search_btn" id="search_btn">Search</button> <img data-toggle="tooltip" title="Click [SEARCH] to show a full list of residents." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;">
 ...
 <?php
         ...
         */
                   ...
                   <?php 
                    ...
                    $key_query = "SELECT c.`customer_id`,c.`first_name`,c.`last_name`,c.`email`,c.`property_id`,c.`valid_from`,c.`room_apt_no`,c.`first_login_date`,c.`vlan_id`,c.`username`,c.valid_until,c.valid_from,c.validity_time,count(d.`mac_address`) AS countd
...
<div class="tab-pane <?php if($active_tab=="session"){echo"active";}?>" id="session">
      ...
      ?>
       <h1 class="head"><span>
    Session Search <img data-toggle="tooltip" title="You can search for a residents online sessions using Email, First Name, Last Name or device MAC Address. To get a list of all sessions, click the Search Button." src="layout/ATT/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;"></span>
</h1>

      <form id="edit-profile" action="?tab=session" method="post" class="form-horizontal middle" >
        <?php
        ...
        <fieldset>
          ...
          <div class="control-group middle-large">
                                     ...
                                     <div class="controls">
                                      ...
                                      <label class="" for="radiobtns">Search Residents <img data-toggle="tooltip" title="You can search for resident sessions using full or partial First Name, Last Name, or Email or use the Search MAC field." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></label>
                                     ...
                                     <div class="controls">
                                      ...
                                      <label class="" style="margin-top: 10px" for="radiobtns">Search MAC <img data-toggle="tooltip" title="You can search for resident sessions using MAC address of a device or use the Search Tenant field." src="layout/ATT/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"></label>
...
<script type="text/javascript" src="js/bootstrapValidator.js"></script>
  ...
  <script type="text/javascript">
    ...
    $('#device_pass_form').bootstrapValidator({
