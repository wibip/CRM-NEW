...
function syncCustomernew($Response1=NULL,$Response2=NULL,$search_id_up,$acc_type=NULL){
		...
		$bulk_profile_data=$bulk_profile_data1['Description'];
		/*
		$bulk_profile_datasession[1] = uniqueAssocArray($bulk_profile_datasession[1], 'Mac', 'Realm');
		$bulk_profile_datasession[0] = uniqueAssocArray($bulk_profile_datasession[0], 'Mac', 'Realm');*/

		$res_arrnew=array_merge($bulk_profile_data[0],$bulk_profile_datasession[0]);
		//echo $res_arrnew[0]['Mac'];
		//print_r($bulk_profile_data[0]);
		
		$resultArray = uniqueAssocArray($res_arrnew, 'Mac', 'Realm');
		$finalarray=array($resultArray,$bulk_profile_datasession[1],$bulk_profile_data[1]);
		//$finalarray=array($res_arr1,$res_arr2);
...
// echo 'src/AAA/'.$network_profile.'/index.php';
		...
		if($newarray['status_code']=='200'){
			...
			if($newarray2['status_code']=='200'){
				...
				if(true){
					...
					$_SESSION[add_msg1] = "<div class='alert alert-success' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$add_mac_sessions_msg."</strong></div>";
				}else{
                    $add_mac_sessions_msg = $message_functions->showNameMessage('device_mac_add_failed',$add_mac,'2009');

					$_SESSION[add_msg2] = "<div class='alert alert-danger' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$add_mac_sessions_msg."</strong></div>";
                ...
                $_SESSION[msg1] = "<div class='alert alert-danger' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$add_mac_sessions_msg."</strong></div>";
            }else{
                $add_mac_sessions_msg = $message_functions->showNameMessage('device_mac_add_failed',$add_mac,'2009');
				$_SESSION[add_msg3] = "<div class='alert alert-danger' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$add_mac_sessions_msg."</strong></div>";
			}
		}
		elseif($newarray['status_code']=='409'){
			...
			$_SESSION[msg1] = "<div class='alert alert-warning' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$add_mac_sessions_msg."</strong></div>";
		}
	}
	...
	if(isset($_GET['rm_device_mac'])){
		if($_GET['token']==$_SESSION['FORM_SECRET']){
			...
			if($newarray['status_code']=='200'){
				...
				$_SESSION[add_msg1] = "<div class='alert alert-success' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$rm_device_mac_msg."</strong></div>";

			}else{
                $rm_device_mac_msg = $message_functions->showMessage('device_remove_failed','2009');

				$_SESSION[add_msg3] = "<div class='alert alert-danger' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$rm_device_mac_msg."</strong></div>";
			}
		}else{
            $rm_device_mac_msg = $message_functions->showMessage('transection_fail','2004');
			$_SESSION[msg1] = "<div class='alert alert-warning' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'></button><strong>".$rm_device_mac_msg."</strong></div>";
		}
...
// send log id**********************
	if(isset($_POST['submit_log'])){
		if($_POST['token']==$_SESSION['FORM_SECRET']){
			...
			if($mail_sent){
				$_SESSION[msg2] = "<div class='alert alert-success' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'> � </button><strong> Log ID submitted successfully </strong></div>";
			}else{
				$_SESSION[msg2] = "<div class='alert alert-danger' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'> � </button><strong> Log ID submition failed</strong></div>";
			}
		}else{
			$_SESSION[msg2] = "<div class='alert alert-warning' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'> x </button><strong>".$message_functions->showMessage('transection_fail','2004')."</strong></div>";
	...
	if(isset($_GET['rm_session_mac'])){
		if($_GET['token']==$_SESSION['FORM_SECRET']){
			 ...
			 $removed_session_array = array();
				 ...
				 if(strlen($rm_session_realm)>0){
					...
					if($newarray['status_code']=='200'){
						...
						// $delete_mac = $mac;
						 $_SESSION[add_msg3] = "<div class='alert alert-success' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'> x </button><strong>".$message_functions->showMessage('session_remove_success')."</strong></div>";

					}
					else{

						$failed = 1;
						$_SESSION[add_msg3] = "<div class='alert alert-danger' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'> x </button><strong>".$message_functions->showMessage('session_remove_fail','2009')."</strong></div>";
			...
			$_SESSION[msg1] = "<div class='alert alert-warning' style='margin-top: 50px;'><button type='button' class='close' data-dismiss='alert'> � </button><strong>".$message_functions->showMessage('transection_fail','2004')."</strong></div>";
	...
	if(isset($_POST['all_search']) || isset($_GET['all_search'])){
		...
		mysql_query("DELETE 
                ...
                if($private_module==1){
		//print_r($sessionresult);
		$result=$nf->checkRealmAccount($pr_realm);
		$search_id=uniqid();
	...
	if(isset($_POST['search_mac_sessions']) || isset($set_search_mac_sessions) || isset($_GET['search_mac_sessions']) ){
		...
		$search_id=uniqid();
                ...
                if($private_module==1){ 
        $result=$nf->checkMacAccount($session_search_mac,$pr_realm);
		//print_r($result);
		$search_id=uniqid();
		$newjsonvalue=syncCustomernew($result,$sessionresult,$search_id,'Private');
                }
									...
									<table id="device_sessions" class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
										...
										if(isset($_POST['search_mac_sessions']) || isset(
											...
											echo '<td>';
														...
														<script type="text/javascript">
                                                            ...
                                                            $(\'#' . $session_row_count . 'DEVICE_DELETE_' . $mac . '\').easyconfirm({locale: {
                                                                ...
                                                                $(\'#' . $session_row_count . 'DEVICE_DELETE_' . $mac . '\').click(function() {
                                                                    window.location = "?token=' . $secret . '&t=1&rm_device_realm='.$sh_realm.'&rm_session_token='.$session_id.'&rm_device_mac=' . $mac . '&' . $next_search_pram . '=' . $mac . '"
                                                                });
                                                                });
                                                        </script>
														';
											 ...
											 echo '<td>'; 
													if(strlen($newstate)>0 && $_GET['rm_session_mac']!=$mac){
													echo'<a href="" id="SESSION_DELETE_' . $mac .'"  class="btn btn-small btn-danger">
														...
														var deleteSessionCheck'.$mac.' = function (){
														    ...
														    if(data == \'0\'){
														    alert(data);
                                                                $(\'#SESSION_DELETE_'.$mac.'\').html(\'<i class="btn-icon-only icon-remove-circle"></i>&nbsp;Deleted\');
										...
										elseif(isset($_POST['all_search']) || isset($_GET['all_search'])){
											...
											foreach ($newjsonvalue[0] as $value2) {
											 ...
											 echo '<td>';
												...
												echo '</td>';
												echo '<td>';
													if(strlen($newstatus)>0 && $_GET['rm_session_mac']!=$mac ){
													echo'<a href="" id="SESSION_DELETE_' . $mac .'"  class="btn btn-small btn-danger">
