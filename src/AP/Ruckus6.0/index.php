...
{
	protected $url_postfix = '/wsg/api/public/v10_0';
	protected $switch_url_postfix = '/switchm/api/v10_0';
    protected $ap;
...
//$this->log(__FUNCTION__,'Check Zones', $url2 ,'GET', $status , $message , '');
	...
	public function createzone($name,$setuname,$setpassword,$domain_id=NULL,$latitude=NULL,$longitude=NULL,$jsonData2=NULL){
		...
		if(!empty($jsonData2)){
			...
			//change according to API Doc
			if(array_key_exists('zoneAffinityProfileId',$jsonData2)){
				$zoneAffinityProfileId = $jsonData2['zoneAffinityProfileId'];
				unset($jsonData2['zoneAffinityProfileId']);
				$jsonData2['dpGroupId'] = $zoneAffinityProfileId;
			}
        }else{
            	$jsonData2 = array(
								'name'   => $name,
...
//////////modify tunnel profile/////
	...
	function modifyTunnelProfile($zoneid,$tunnelProfileID,$tunnelProfileName){
		...
		*/
		...
		foreach ($net_list['list'] as $net){
			//parse_str($this->retrieveOneNetwork($zoneid,$net['id']),$net_details_res);

			//$net_details = json_decode($net_details_res['Description'],true);
			//if($net_details['accessTunnelType']=='SoftGRE'){
