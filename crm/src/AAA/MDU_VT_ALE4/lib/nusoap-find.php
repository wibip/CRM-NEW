...
class nusoap_client extends nusoap_base  {
	...
	* @access   public
	*/
	function call($operation,$params=array(),$sync_limit,$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		$this->operation = $operation;
		$this->sync_limit = $sync_limit;
		$this->fault = false;
		$this->setError('');
		$this->request = '';
		$this->response = '';
			...
			if ($use == 'literal') {
				...
				if ($namespace) {
					...
					$payload = "<prov:$operation accountType=\"$namespace\">" .
								"<prov:queries>".
					            "<prov:query reference=\"getact\" limit=\"".$sync_limit."\">".
					            "<prov:id>".
								$payload .
								"</prov:id>".
								"<prov:parameters>".
	...
	* @access   public
	*/
	function callParms($operation,$params=array(),$params2=array(),$sync_limit,$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		$this->operation = $operation;
		$this->sync_limit = $sync_limit;
		$this->fault = false;
		$this->setError('');
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			$payload2 = '';
			if (is_string($params2)) {
				$this->debug("serializing param string for operation $operation");
				$payload2 = $params2;
			} elseif (is_array($params2)) {
				$this->debug("serializing param array for operation $operation");
				foreach($params2 as $k => $v){
					$payload2 .= $this->serialize_val($v,$k,false,false,false,false,$use);
				}
			} else {
				$this->debug('params must be array or string');
				$this->setError('params must be array or string');
