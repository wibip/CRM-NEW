...
class network_functions
{
	...
	public function masterAcc($username,$password,$organization,$service_profile_product,$first_name,$last_name,$email,$mobile_number,$user_data_string,$validity_time,$country,$address,$state,$city,$zip,$secret_question,$secret_answer,$vlan_id,$passcode_key=null){
		...
		$this->log($username,__FUNCTION__, 'Create Account', $url2, 'POST', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,'');
	...
	/*else{
		...
		$this->log($mac,__FUNCTION__, 'Add Device', $url2, 'POST', $status_code, $body, $jsonDataEncoded,$organization,$macc);
	...
	public function modifysubAcc($mac,$username,$organization,$first_name,$last_name,$email,$mobile_number,$vlan_id=NULL){
		...
		$finalarr['PAS-Last-Name'] = $last_name;
		$finalarr['Product-Owner']=$group;
		if (!empty($vlan_id)) {
			$finalarr['User-Data']['VT-Vlan-Id'] = $vlan_id;
			}
		$jsonDataEncoded=json_encode($finalarr);
		
		$url2 = $this->network('api_base_url').'/accounts/'.$mac;
		...
		$this->log($mac,__FUNCTION__, 'Modify Device', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$macc);
	...
	public function modifyAcc($cust_uname,$email,$password,$first_name,$last_name,$organization,$mobile_number,$user_data_string,$vlan_id){
		...
		$passcode_key=$decodednew['User-Data']['dpsk'];

		if(empty($passcode_key)){
			$passcode_key=$this->getDpskKey($cust_uname);
		}
		...
		if($this->getDpskEnable($organization) && !empty($passcode_key)){
	...
	public function modifyAccDpsk($user_name,$passcode_key){
		...
		$this->log($user_name,__FUNCTION__, 'Update Account', $url2, 'PUT', $status_code, $body, $jsonDataEncoded,$organization);
	...
	public function removeProfile($user_name){
		...
		$this->log($user_name,__FUNCTION__, 'Remove Profile', $url2, 'DELETE', $status_code, $body, '',$organization,$mac);
	...
	public function modifyProfile($user_name,$product_list,$password=NULL){
		...
		$passcode_key=$decodednew['User-Data']['dpsk'];
		if(empty($passcode_key)){
			$passcode_key=$this->getDpskKey($user_name);
		}
...
//		$organization=$decoded['Group'];
		...
		$this->log($user_name,__FUNCTION__, 'Modify Profile', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$mac);
...
*/
		...
		$passcode_key=$newarr['User-Data']['dpsk'];
		if(empty($passcode_key)){
			$passcode_key=$this->getDpskKey($user_name);
		}
...
//		$organization=$decoded['Group'];
		...
		$this->log($user_name,__FUNCTION__, 'Modify QOS Profile', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$mac);
...
//		$organization=$decoded['Group'];
	...
	public function modifyValidityTime($user_name,$validity_time){
		...
		$this->log($user_name,__FUNCTION__, 'Update Account', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$mac);
	...
	public function AUP($user_name,$user_message_text,$aup_message_text){
		...
		$this->log($user_name,__FUNCTION__, 'AUP Modify Account', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$mac);
	...
	public function UpdateProduct($user_name,$product){
		...
		$this->log($user_name,__FUNCTION__, 'Update Product', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$mac);
	...
	public function AUP_Activate($user_name,$violation_count=null){
		...
		$this->log($user_name,__FUNCTION__, 'AUP Activate', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,$mac);
	...
	public function UserMessage($user_name,$user_message_text){
		...
		$this->log($user_name,__FUNCTION__, 'Update Account', $url2, 'PUT', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$group,$mac);
	...
	public function checkMacAccount($user_name){
		...
		$this->log($user_name,__FUNCTION__, 'Get Device', $url2, 'GET', $status_code, $body, '',$organization,$mac);
	...
	public function checkMasterAccount($user_name){
		...
		$this->log($user_name,__FUNCTION__, 'Get Account', $url2, 'GET', $status_code, $body, '',$organization,$mac);
	...
	public function findUsers($search_parameter1, $term1,$realm=null){
		...
		$this->log($term1,__FUNCTION__, 'Get Account', $url2, 'GET', $status_code, $body, '',$realm,$mac);
	...
	public function findDeviceUsers($search_parameter1, $term1,$realm=null){
		...
		$this->log($realm,__FUNCTION__, 'Get Account', $url2, 'GET', $status_code, $body, '',$organization,$mac);
	...
	public function getGroup(){
		...
		$this->log($cust_uname,__FUNCTION__, 'Search Group', $url2, 'GET', $status_code, $this->db_class->escapeDB($body), '',$organization,$mac);
	...
	public function findMasterUsers($search_parameter1, $term1,$search_parameter2, $term2,$property=null){
		...
		$this->log($term2,__FUNCTION__, 'Get Account', $url2, 'GET', $status_code, $this->db_class->escapeDB($body), '',$property,$mac);
    ...
    public function findMasterUsersByParams($search_parameter1, $term1,$search_parameter2, $term2,$search_parameter3, $term3){
		...
		$this->log($params2,__FUNCTION__, 'Get Account', $url2, 'GET', $status_code, $this->db_class->escapeDB($body), '',$organization,$mac);
...
/* 		$fields = array(
		...
		$group=$this->network('api_master_acc_type');
		if (strlen($group)<1) {
			$group=$this->network('api_acc_profile');
		}
		// This is an archaic parameter list
        ...
        //$organization=$decoded['Group'];
		$this->log($master_user,__FUNCTION__, 'Get Device', $url2, 'GET', $status_code, $this->db_class->escapeDB($body), '',$organization,$mac);

		$session_response = $this->getDeviceSessions($master_user);
		$newdecoded = json_decode($session_response, true);
		
		if (empty($newdecoded)) {
			$session_response = $this->getSessions($organization,'Access-Group',$property_id);
	...
	public function delUser($network_user){
		...
		$this->log($network_user,__FUNCTION__, 'Delete Account', $url2, 'DELETE', $status_code, $this->db_class->escapeDB($body), '',$organization,$mac);
	...
	public function getSessionsbymac($username,$organization=null){
		...
		$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter[Access-Group]='.$organization.'*&filter[Sender-Type]=Aptilo-AC&limit=10000';
		...
		$this->lognew(__FUNCTION__, 'Device Session Search', $url2, 'GET', $status_code,$this->db_class->escapeDB($body), '' ,$organization,$mac);
	...
	public function getDeviceSessions($master_uname){
		...
		//$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter[Access-Group]='.$organization;
		$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter[AAA-User-Name]='.$master_uname.'&filter[Sender-Type]=Aptilo-AC&filter[Access-Profile]=FULL&limit=10000';
		...
		$this->lognew(__FUNCTION__, 'Session Search', $url2, 'GET', $status_code,$this->db_class->escapeDB($body), '' ,$organization,$master_uname);
	...
	public function getVtenantSessionsbyrealm($realm, $time_zone){
		...
		//$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter[Access-Group]='.$organization;
		$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter[Access-Group]='.$realm.'*&filter[Sender-Type]=Aptilo-AC&filter[Access-Profile]=FULL&limit=10000';
	...
	public function getSessionsDevice($username,$parameter,$property_id=null){
		...
		$group=$this->network('api_acc_profile');
			...
			$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter['.$parameter.']='.$username.'&limit=10000';
	...
	public function getSessions($username,$parameter,$property_id=null){
		...
		if ($parameter=='AAA-User-Name') {
			$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter['.$parameter.']='.$username.'*&filter[Access-Profile]=FULL&filter[Sender-Type]=Aptilo-AC&limit=10000';
		}
		else{
			$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter['.$parameter.']='.$username.'&filter[Sender-Type]=Aptilo-AC&limit=10000';
		}
	...
	public function getSessionsRuckus($username,$parameter){
		...
		//$user_name='FrontierMagnusBlr';
		$group=$this->network('api_acc_profile');
		$url2 =$this->network('api_base_url').'/sessions?filter[Zone]='.$group.'*&filter['.$parameter.']='.$username.'&filter[Sender-Type]=Ruckus-SCG&limit=10000';
	...
	public function delSessions($session_token){
        ...
        $status_code = curl_getinfo($ch2, CURLINFO_HTTP_CODE);
				...
				if($status_code == '204'){
				$this->lognew(__FUNCTION__, 'Session Delete', $url2, 'DELETE', $status_code,'204 = deletion is success', '' ,$group,$mac);
				return 'status=success&status_code='.'200'.'&Description=Session deleted Success - '.$status_code;
	
				}
				else{
				$this->lognew(__FUNCTION__, 'Session Delete', $url2, 'DELETE', $status_code,$this->db_class->escapeDB($body), '' ,$group,$mac);
					return 'status=failed&status_code='.$status_code.'&Description='.$desc.' - '.$status_code;
					/* SOAP Parameter Error */
				}
	...
	$this->log($product, __FUNCTION__, 'Get Product', $url2, 'GET', $status, $this->db_class->escapeDB($body), '', $organization,$mac);
	...
	$this->log($product, __FUNCTION__, 'Get QOS', $url2, 'GET', $status, $this->db_class->escapeDB($body), '', $organization,$mac);
	...
	public function getSessionbytoken($mac,$session_token,$group){
		...
		$status = curl_getinfo($ch, CURLINFO_HTTP_CODE);
		$this->lognew(__FUNCTION__, 'Session Search', $url2, 'GET', $status,$this->db_class->escapeDB($body), '' ,$group,$mac);
	...
	public function disconnectUserSessions($session_token,$user_name){
        ...
        $decoded = json_decode($body, true);
				...
				if($status_code == '204'){
				$this->lognew(__FUNCTION__, 'Session Delete', $url2, 'DELETE', $status_code,'204 = deletion is success', '' ,$organization,$mac);
					return 'status=success&status_code='.'200'.'&Description=Session deleted Success - '.$status_code;
	
				}
				else{
				$this->lognew(__FUNCTION__, 'Session Delete', $url2, 'DELETE', $status_code,$this->db_class->escapeDB($body), '' ,$organization,$mac);
					return 'status=failed&status_code='.$status_code.'&Description='.$desc.' - '.$status_code;
					/* SOAP Parameter Error */
				}
	...
	$jsonDataEncoded=json_encode($params_update);
		...
		$this->lognew(__FUNCTION__, 'Session Start', $log_data, 'POST', $status_code,$this->db_class->escapeDB($body), $log_data ,$group,$mac);
	...
	public function  authenticate_user($username,$password,$organization){
		...
		$url2 = $this->network('api_base_url').'/accounts/'.$username.'/authenticate';
		...
		$this->log($username,__FUNCTION__, 'Authenticate User', $url2, 'POST', $status_code, $this->db_class->escapeDB($body), $jsonDataEncoded,$organization,'');
...
public function getDpskEnable($property_id){

	$advanced_features_di = $this->db_class->getValueAsf("SELECT p.`features` AS f FROM 
	`mno_distributor_parent` p JOIN `exp_mno_distributor` d ON p.`parent_id`=d.`parent_id` 
	JOIN `mdu_distributor_organizations` m ON d.`distributor_code`= m.`distributor_code`
	WHERE m.`property_id`='$property_id'");

$af_decode =  json_decode($advanced_features_di, true);

$dpsk_enable = false;
if(array_key_exists('DPSK', $af_decode)){ 
...
public function getDpskKey($cust_uname){


return $this->db_class->getValueAsf("SELECT `dpsk_key` AS f FROM `mdu_vetenant` WHERE `username` = '$cust_uname'");
