...
$auth_trusted_origin = $package_functions->getOptions("AUTH_TRUSTED_ORIGIN", $admin_system_package);
if($auth_trusted_origin != 'N/A'){
	$array_trusted_zones = explode(',',$auth_trusted_origin);
	/// Validate Trusted Source ///
	if(!(in_array($origin,$array_trusted_zones))){
		//echo 'Invalid Origin';
		$oops_error_message = $message_functions->showMessage('auth_untrusted_origin');
		include_once 'layout/'.$oops_error_layout.'/views/saml_error.php';
		logs_saml('Invalid Origin','400');
		exit();
	}
}
///////////////////////
