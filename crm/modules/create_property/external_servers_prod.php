	  ...
	  if($isDynamic){
		...
		if(!$reseller_automation_on){
			...
			if (empty($external_arr)) {
				$external_arr = array('vsz' => array('auto' =>  'ruckusautomation',
											'manual' =>  'ruckusmanual') );
			}

			if($this->isReadyTv){
				$external_arr = array('vsz' => array('auto' =>  'ruckusautomation',
				'manual' =>  'ruckusmanual'), 'zabbix'=>'zabbix' );
			}
			
		}else{
			$external_arr = array('vsz' => array('auto' =>  'ruckusautomation',
											'manual' =>  'ruckusmanual'), 'aaa' =>'alematchentry','zabbix'=>'zabbix' );
		}
		$ale = false;
		$vszauto = false;
...
public function cloudpathapi(){
    ...
    $internet_ssid="CloudPath".rand(10,100);
    $ssid_list = array();
         ...
         foreach ($query_results_ss['data'] AS $row_s) {
                ...
                if($police_del_sts == '200' || empty($current_police_id)){
                    ...
                    if ($pdsk_policy_res_data['status_code']=='200') {
                        $query_dpsk_pool="UPDATE `mdu_dpsk_pool` SET `displayName` = '$pool_name' , `description` = '$pool_name' , `ssidList` = '$ssid_list_json' , `device_count` = '$device_limit' , `property_id` = '$dpsk_property_id',`assign_policy` = '$this->dpsk_policies' WHERE  `distributor` = '$dpsk_distributor_code' ";
                        $this->db->execDB($query_dpsk_pool);
                    }
...
public function ruckusautomation(){
                    ...
                    if($create_zone_arr['status_code']=='200'){
                        ...
                        if($zoneq){
                            ...
                            for ($i=1; $i <= (int)$pvt_wlan_count ; $i++) { 
                                ...
                                $createnetwork = $wag_obj->createnetwork($zoneid,'Private-'.$i,$ssid,$add_data,$configFileArr['Private_vlan_id'],$configFileArr['private_accounting']);   
                       ...
                       VALUES ('$reg_id','','DNS-API-Regular','$this->ap_controller','DNS-API-Regular','$reg_primaryIp','$reg_secondaryIp','','','','','','API','$search_id')";
                                    ...
                                    if($createdhcpProfile_arr['status_code']=='200'){}else{
    ...
    public function zabbix(){
    	include_once 'src/zabbix/index.php';
    	$inputarr=$this->inputarr;
    	$status_code = 200;
