...
<div class="tab-pane <?php if (isset($tab_manageTheme)) {
                            echo 'active';
                        } ?>" id="manageTheme">
    ...
    <h1 class="head">
        ...
        make yours a splash page. <img data-toggle="tooltip" title="Guests will automatically be redirected to your customizable webpage upon connection to your Guest WiFi network. Create multiple themes for your location and easily enable/disable stored themes from the “Manage” tab." src="layout/OPTIMUM_ENT/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;">
    </h1>

    <div class="widget widget-table action-table" style="margin-top: 20px;">
        ...
        <div class="widget-content table_response" id="campcreate_div1">
            <div style="overflow-x:auto">
                ...
                <table id="active_theme_table" class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
                    ...
                    <thead>
                        ...
                        <tr>
                            ...
                            <th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="10" style="min-width: 285px;">Clone</th>
                    ...
                    <tbody>
                        ...
                        $theme_arr = active_theme::inst()->multiSSIDActiveThemes($user_distributor);
                        ...
                        foreach ($query_results['data'] as $row) {
                            ...
                            //echo '<a id="RE_' . $theme_id . '"  class="btn btn-small btn-danger" ' . ($is_enable == 1 ? 'disabled' : '') . '><i class="btn-icon-only icon-trash"></i>Remove&nbsp;</a>';
                            ...
                            echo '<td style="min-width: 120px !important;"><a class="btn btn-small btn-danger" target="_blank" href="' . $them_url . '">Test</a>';
                            ...
                            echo '</td>';
                            echo '<td><a id="save_as_' . $theme_id . '" onclick="saveAsShow(\'' . $theme_id . '\')" class="btn btn-small btn-primary">Clone</a><div id="save_div_' . $theme_id . '" style="display:none"><div class="theme_duplicate"><input id="save_input_' . $theme_id . '" type="text" placeholder="' . ucwords(__THEME_TEXT__) . ' Name" class="span3"><a onclick="saveAsShow(\'' . $theme_id . '\')" class="btn btn-small btn-primary" style="vertical-align: super;padding: 0px 8px !important;right: 50px;"><span>Cancel</span></a><a id="dup_' . $theme_id . '" class="btn btn-small btn-primary" style="vertical-align: super;margin-left: 10px;"><span>Save</span></a><div class="dup_loader"></div><script type="text/javascript">

                                                            $(document).ready(function() {

                                                            $(\'#dup_' . $theme_id . '\').easyconfirm({locale: {

                                                                    title: \'Save ' . ucwords(__THEME_TEXT__) . '\',

                                                                    text: \'Are you sure,you want to save this ' . __THEME_TEXT__ . '?&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\',
                                                                ...
                                                                $(\'#dup_' . $theme_id . '\').click(function() {
                                                                    $(".dup_loader").addClass("loading");
                                                                    var tname = $("#save_input_' . $theme_id . '").val();
                                                                    if(tname){
                                                                        $("#dup_req_' . $theme_id . '").hide();
                                                                        $.ajax({
                                                                            type: "POST",
                                                                            url: "ajax/validateThemeName.php",
                                                                            data: {user_distributor: "' . $user_distributor . '",tname:tname},
                                                                            success: function(data) {
                                                            
                                                                            if(data >0){
                                                                                $("#dup_exists_' . $theme_id . '").show();
                                                                                $(".dup_loader").removeClass("loading");
                                                                                return false;
                                                                            }else{
                                                                                $("#dup_exists_' . $theme_id . '").hide();
                                                                                window.location = "?t=manageTheme&token=' . $secret . '&duplicate=1&id=' . $theme_id . '&theme_name="+tname+"&lan=' . $LANGUAGE . '";
                                                                            }
                                                                        }
                                                                        });
                                                                    }else{
                                                                        $("#dup_req_' . $theme_id . '").show();
                                                                        $(".dup_loader").removeClass("loading");
                                                                        return false;
                                                                    }
                                                                ...
                                                                $(\'#save_input_' . $theme_id . '\').on(\'keyup\', function(event) {
                                                                    $("#dup_exists_' . $theme_id . '").hide();
                                                                    if($(this).val()){
                                                                        $("#dup_req_' . $theme_id . '").hide();
                                                                    }else{
                                                            ...
                                                            </script></div><small id="dup_req_' . $theme_id . '" class="help-block" style="display: none;"><p>This is a required field</p></small><small id="dup_exists_' . $theme_id . '" class="help-block" style="display: none;"><p>' . ucwords(__THEME_TEXT__) . ' name already exists</p></small></div></td>';
                ...
                <script>
                    function saveAsShow(theme_id) {
                        $('#save_as_' + theme_id + ', #save_div_' + theme_id).toggle();
                    }
                </script>
                <style>
                    .dup_loader {
                    ...
                    .theme_duplicate input {
                        margin: 0;
                        padding-right: 120px;
                        width: 100% !important;
                    }

                    .table .theme_duplicate a.btn:not(.toggle-on):not(.toggle):not(.toggle-off):not(.toggle-handle) {
                        position: absolute;
                        right: 0px;
                        width: 50px;
