                ...
                <table id="active_theme_table" class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
                    <thead>
                        <tr>
                            ...
                            <th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="1">TEST</th>
                    ...
                    <tbody>
                        ...
                        $key_query = "SELECT id,ref_id,theme_id, theme_name,title, `LANGUAGE`, registration_type,create_date,is_enable,language_string,template_name FROM exp_themes
                        ...
                        foreach ($query_results['data'] AS $row) {
                            ...
                            echo '</td><td>';
                            echo '<a class="btn btn-info" target="_blank" href="plugins/theme_create/themeView.php?reg_type=' . $registration_type . '&template_name=' . $template_name . '&dist=' . $user_distributor . '&theme_id=' . $theme_id . '&enc=&action=view&title='.$title.'">Preview</a>';
                            echo '</td><td>';
                            
                            echo '<a class="btn btn-info" target="_blank" href="'.$base_folder_path_preview.'/checkpoint'.$extension.'?'.$db->getValueAsf("SELECT n.group_parameter AS f  FROM exp_network_profile n , exp_settings s WHERE s.settings_value = n.network_profile").'='.$db->getValueAsf("SELECT `verification_number` AS f FROM `exp_mno_distributor` WHERE `distributor_code`='$user_distributor' LIMIT 1").'&'.$mac_parameter.'=DEMO_MAC&theme='.$theme_id.'">Test</a>';
                            echo '</td><td>';
                            echo '<a id="CM_' . $theme_id . '"  class="btn  btn-primary"><i class="btn-icon-only icon-wrench"></i>Edit&nbsp;</a><script type="text/javascript">
                            ...
                            echo '<a id="RE_' . $theme_id . '"  class="btn  btn-danger"><i class="btn-icon-only icon-trash"></i>Remove&nbsp;</a>
                                                                ';/*JavaScript part move in to is_enable if condition else part*/

                            if (in_array("THEME_IMPORT", $features_array) || $package_features == "all") {
                                echo '<a id="DOWN_' . $theme_id . '"  class="btn  btn-warning"><i class="btn-icon-only icon-download-alt"></i>Export</a><script type="text/javascript">
