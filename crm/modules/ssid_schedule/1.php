<div <?php if(isset($tab_guestnet_schedule)){?>class="tab-pane fade in active" <?php }else {?> class="tab-pane fade" <?php }?>  id="guestnet_schedule">
    ...
    $retrieveSchedulerCache = [];
    $gst_wifi_schedule_intro=$message_functions->getPageContent('guest_wifi_schedule_content',$system_package);
...
<h1 class="head container">SSID Schedule</h1>
    <p>Conserve energy and enable controls for your employees and guests who have access to your network.</p>
    <p>With the Scheduler, you can control the hours that your employees and guests will be able to connect to your <?php echo __WIFI_TEXT__?> network. By setting when your SSID is turned on and off, you can reduce energy consumption by only enabling it during your business hours. Turning off your SSID puts the Access Point in "low power mode", which reduces the power consumption from an already low 10W to an almost insignificant 2.5W.</p>
    <p>To set a schedule for one of your SSID use the sliders to set the schedule between 12am and 11pm. Once you are satisfied click the "Set" button to enable the schedule. Sliding the sliders to opposite ends enables "Always On". Sliding one slider on top of other enables "Always Off".</p>
    ...
    if(method_exists($wag_obj,'retrieveSchedule')){
        ...
        if($ssid_res['rowCount']>0){
            ...
            $status_code="0";
                                                            ...
                                                            parse_str($schedule_API_sesponce, $shedule_response);
                                                            array_push($retrieveSchedulerCache,array("zone"=>$zone_id,"data"=>$shedule_response));
                                                            if($shedule_response['status_code']==200){
                                                                ...
                                                                //print_r( $Description );
                                                                $Description=(array)json_decode(urldecode($shedule_response['Description']));
                                                                ...
                                                                for($x=0;$x<$schedule_list_count;$x++){
                                                                    ...
                                                                    $shedule_exists=$db->getValueAsf("SELECT `uniqu_name` AS f FROM `exp_distributor_network_schedul` WHERE `uniqu_name`='$new_schedule_id' GROUP BY`uniqu_name`");
            ...
            foreach ($ssid_res['data'] as $row){
                ...
                $wlan_called = 0;
                foreach ($retrieveOneNetworkCache as $value) {
                    if($value['wlan']==$WLanID){
                        $wlan_called = 1;
                        $WLan_respo = $value['data'];
                        break;
                    }
                }
                ...
                switch($WLan_schedule_type){
                    ...
                    case 'AlwaysOn' : {
                        ...
                        $zone_called = 0;
                        foreach ($retrieveSchedulerCache as $value) {
                            if($value['zone']==$zone_id){
                                $zone_called = 1;
                                $schedule_in_controller = $value['data'];
                                break;
                            }
                        }
                ...
                <form id="tab5_form<?php echo$sID; ?>" name="tab5_form1" method="post" action="modules/ssid_schedule/1-submit?t=guestnet_schedule&script=<?php echo $script; ?>" class="form-horizontal">
                    ...
                    <fieldset class="fieset">
                        ...
                        <div>
                            ...
                            <div class="power_box2" style="height: 469px !important;width: 33%;display: inline-block;    background: #d7d7d7;box-sizing: border-box;padding: 20px">
                                <h2>SSID :  <br>
