...
<div <?php if (isset($tab_tier_support)) { ?>class="tab-pane fade in active" <?php } else { ?> class="tab-pane fade" <?php } ?> id="tier_support" style="margin-top: 30px;">
	...
	<br />
	<?php $q_network_realms = "SELECT network_realm,wLan_id,s.ssid FROM exp_network_realm nr
	...
	<form action="?t=tier_support" id="mac_search" class="form-horizontal" method="post">
		<fieldset>
			<div class="control-group">
				<div class="controls form-group form_control_n">
					<?php
					if ($network_realms['rowCount'] < 1 || !$multi_ssid_venue) {
					...
					?>
						...
						<img id="tooltip1" data-toggle="tooltip" title="Before searching for a specific device you must first select a realm. You can list all devices across all realms it a specific realm." src="layout/OPTIMUM/img/help.png" style="width: 30px;margin-bottom: 0px;cursor: pointer;display:  inline-block;">
					<?php } ?>
				</div>
			</div>
			<div class="control-group">
	...
	<form action="?t=tier_support" class="form-horizontal" method="post" id="add_mac_form" name="add_mac_form">
		...
		<fieldset>
			...
			<div class="control-group">
				<div class="controls form-group" style="margin-left: 0px !important;">
					<?php
					...
					?>
						<select name="net_realm" class="span2">
							<?php
							foreach ($network_realms['data'] as $row) {
								echo '<option value="' . $row['network_realm'] . '">' . $row['ssid'] . '</option>';
							}
			...
			<div class="control-group">
				...
				<div class="controls form-group form_control_n" style="margin-left: 0px !important;">
					...
					<input type="text" name="add_mac" class="span3" id="add_mac" maxlength="17" placeholder="xx:xx:xx:xx:xx:xx" data-fv-regexp="true" autocomplete="off" data-toggle="tooltip" title="The MAC Address can be added manually or you can paste in an address. Acceptable formats are: aa:bb:cc:dd:ee:ff, aa-bb-cc-dd-ee-ff, or aabbccddeeff in lowercase or all CAPS." onkeypress="return event.keyCode != 13;" />
