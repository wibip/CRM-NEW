...
$suspend_location_view=$package_functions->getOptions('SUSPEND_LOCATION_VIEW',$system_package);
                         ...
                         $location_q = "SELECT DISTINCT  d.`is_enable` AS loc_is_enable,d.`gateway_type`,d.`private_gateway_type`,d.id AS dis_id,d.`distributor_code`,d.`distributor_name`,d.`network_type`,d.`distributor_type` ,d.`state_region` ,d.`mno_id`,d.`country`,u.id AS userid,u.`verification_number`,u.is_enable,u.user_name
                                                                                FROM `exp_mno_distributor` d
                                                                                LEFT JOIN admin_users u ON d.`distributor_code`=u.`user_distributor`
                                                                                WHERE d.parent_id = '$get_edit_parent_id'
