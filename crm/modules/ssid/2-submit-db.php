...
if(isset($_GET['ssid_broadcast']) && $_SESSION['guestnet_tab_1_MOD_SECRET']==$_GET['secret']){
    ...
    if($private_modify_sche_res['status_code']=='200'){
        ...
        $query_results3=mysql_query($selq3);
        ...
        $query_results4=mysql_query($selq4);
...
elseif (isset($_GET['enable_p_schedule'])){
    ...
    $ssid_broadcast = $db_class->getValueAsf("SELECT shedule_uniqu_id as f FROM exp_distributor_network_schedul_assign WHERE distributor='$user->user_distributor' AND network_id='$network_id'");

    $ap = new apClass();
    $wag_obj = $ap->getControllerInst();
    $zone_id = $ap->getZoneId();

    $active_schedule_name=$db_class->getValueAsf("SELECT s.uniqu_name AS f  FROM exp_distributor_network_schedul s
