...
<div <?php if (isset($tab_manage_SSID_splash)) { ?>class="tab-pane fade in active" <?php } else { ?> class="tab-pane fade" <?php } ?> id="manage_SSID_splash">
    ...
    ?>
        ...
        <h2 class="head">
            Manage SSID by Service Area. <img data-toggle="tooltip" title="<?php echo $tooltip; ?>" src="layout/OPTIMUM/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;">
        </h2>
    ...
    <div class="widget widget-table action-table">
        ...
        <div class=" widget-table widget-content table_response " style="position: relative;">
            <div style="overflow-x:auto;">
                ...
                <table id="group_tag_table" class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
                    ...
                    <tbody>
                        ...
                        /*$queryall = "SELECT ss.`id` AS 'id_new',ss.`group_tag` AS 'group_tag',elas.`group_tag` AS 'group_tag2',ss.wlan_group_id AS 'Wlan_group',ss.`network_id` AS 'Network',ss.`ap_group_id` AS 'AP_group',g.theme_name
                                                    FROM `exp_distributor_wlan_group_ssid` ss JOIN exp_locations_ssid s ON ss.network_id=s.network_id AND s.distributor=ss.distributor LEFT JOIN `exp_mno_distributor_group_tag` g ON  ss.`group_tag`= g.`tag_name`
                                                    LEFT JOIN exp_locations_ap_ssid elas ON g.tag_name=elas.group_tag AND elas.distributor=g.distributor AND s.ssid=elas.location_ssid
                                                    WHERE ss.`distributor`='$user_distributor' ORDER BY ss.`ap_group_id`,s.`ssid`";*/
...
ORDER BY ss.`ap_group_id`, s.`ssid`";
                        ...
                        foreach ($results['data'] as $rows) {
                            ...
                            //                    if($guest_gateway!='AC'){
                            $ap_group_count = count($resultnew['data']);
                            ...
                            //                    if ($guest_gateway!='AC') {
                            $ssid_count = count($resultnew2['data']);
                            $rowcountn = $ap_group_count * $ssid_count;

                            //                    }else{
                            //                        $rowcountn = count($resultnew2['data']);
                            //                    }
                            if ($i == ($a - 1) && (count($existssid) < $rowcountn)) {
                    ...
                    function test(elm) {
                        ...
                        if (ap_group_id == "") {
                            ...
                            $.ajax({
                                ...
                                data: {
                                    ap_group_id: ap_group_id,
                                    ap_ssid_id: ap_ssid_id,
                                    zone_id: zone_id,
                                    splash_theme_id: splash_theme_id,
                                    ap_control_var: "<?php echo $ap_control_var; ?>",
                                    ackey: scrt_var,
                                    user_distributor: "<?php echo $user_distributor; ?>"
                                },
