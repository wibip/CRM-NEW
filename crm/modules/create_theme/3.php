...
$gt_template_optioncode= $pack_func->getOptions('TEMPLATE_ACTIVE',$sys_pack);
...
echo $pack_func->getOptions('splash_page_url',$system_package); ?>"
...
<?php 
$query_resultsn=$db->selectDB($key_query_t_name);


foreach ($query_resultsn['data'] AS $row) {
$temp_idn = $row[template_id];
$template_enn=$db->getValueAsf("SELECT `enable_reg_type` as f  FROM exp_template WHERE `is_enable` ='1' AND template_id='$temp_idn'");
$enable_piec = explode(",", $template_enn);
$array_new=array();
foreach ($enable_piec as $value) {
$reg_name=$db->getValueAsf("SELECT `theme_name` as f  FROM exp_theme_manager WHERE theme_code='$value'");
$enable_type['code']=$value;
$enable_type['name']=$reg_name;
array_push($array_new, $enable_type);
}
//$type_ar = array($temp_idn => $enable_piec);
//print_r($type_ar);
$type_arr[$temp_idn]=$array_new;
...
echo '<option id="'.$template_display_name.'" value="'.$edit_template_name.'">'.$template_display_name.'</option>';
			...
			//	echo '<option id="'.$edit_template_name.'" value="'.$edit_template_name.'">'.$edit_template_name.'</option>';
                            ...
                            foreach ($query_results1['data'] AS $row) {
								...
								if ($i==0) {
									$temp_id_active = $row[template_id];
								}
...
$('#reg_type').empty();
var reg_json = '<?php echo json_encode($type_arr); ?>';
var reg_array = JSON.parse(reg_json);

var apend_ob = reg_array[x];
											

apend_ob.forEach(function(element) {
//alert(element['code']);
$("#reg_type").append('<option value="'+element['code']+'">'+element['name']+'</option>');
...
if(x=="default_theme" || x=="red_template" || x=="altice_business_default" || x=="frontier_business_default" || x=="frontier_business_default_new"  || x=="arris_business_default" || x=="att_business_default"){
...
if(x=="red_template" || x=="altice_business_default" || x=="frontier_business_default" || x=="frontier_business_default_new"  || x=="arris_business_default" || x=="att_business_default"){
...
var y = '<img id="preview_img" src="<?php echo $base_portal_folder; ?>/template/'+x+'/img/'+xx+'.jpg?v=15" data-zoom-image="<?php echo $base_portal_folder; ?>/template/'+x+'/img/'+xx+'.jpg?v=15" style="max-height:200px;width:60%;max-width: 100%"><p><a id="pre_id" class="fancybox fancybox.iframe" href="<?php echo $base_portal_folder; ?>/checkpoint.php?client_mac=DEMO_MAC&theme='+template_name1+'&realm=<?php echo $db->getValueAsf("SELECT `verification_number` AS f FROM `exp_mno_distributor` WHERE `distributor_code`='$user_distributor' LIMIT 1");?>">Preview Template Layout</a>';
...
if(x=="CLICK" || x=="PREMIUM" || x=="WOW_GUEST"){
...
else if(x=="MANUAL" || x=="MANUAL_PREMIUM"){
...
else if(conceptName=="CLICK" || conceptName=="PREMIUM" || conceptName=="WOW_GUEST"){
...
else if(conceptName=="MANUAL" || conceptName=="MANUAL_PREMIUM"){
...
if($modify_mode == 1) {
//echo $edit_template_name;
$template_en=$db->getValueAsf("SELECT `enable_reg_type` as f  FROM exp_template WHERE `is_enable` ='1' AND template_id='$edit_template_name'");
}
else{
//echo $temp_id_active;
$template_en=$db->getValueAsf("SELECT `enable_reg_type` as f  FROM exp_template WHERE `is_enable` ='1' AND template_id='$temp_id_active'");
}
$enable_pieces = explode(",", $template_en);


//$gt_reg_optioncode= $pack_func->getOptions('THEME_REG_TYPE',$sys_pack,'MVNO');
$gt_reg_optioncode= $pack_func->getOptions('THEME_REG_TYPE',$sys_pack);
...
if (in_array($pieces[$i], $enable_pieces)) {
if($i==($len-1)){
...
//$back_or_logo=$package_functions->getOptions('LOGO_OR_BACK',$sys_pack);
...
<?php }else{ ?>
		...
		<div id="logo1_up_bc">
			...
			<div class="slider_change" id="slider_change"></div>
...
onAfterImgCrop:function(){ $('.cropControlUpload').show(); $('.new_upload').remove(); $(".cropControlsUpload").removeClass('warning_img'); $('#background_img_check').val('1'); $('#logo1_up_bc').prepend('<div class="slider_change" id="slider_change"></div>'); setColor($('.slider').slider("option", "value"));deleteImg(img_id);console.log('DeletedonAfterImgCrop') },
...
$alernative_text=(array)json_decode($package_functions->getOptions('THEME_ALTERNATIVE_TEXT',$sys_pack));
...
<div id="field_color" class="control-group feild">
			...
			<div class="controls">
				...
				<h3 style="display: inline-block;">Field Color<img data-toggle="tooltip" title="You can set the color of the fields depending on the background used." src="layout/<?php echo $camp_layout; ?>/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;display: inline-block;"></h3>
		...
		<div id="field_txt_color" class="control-group feild">
			...
			<div class="controls">
				...
				<h3 style="display: inline-block;">Field Text Color<img data-toggle="tooltip" title="You can set the color of the field text based on the field color used." src="layout/<?php echo $camp_layout; ?>/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;display: inline-block;"></h3>
