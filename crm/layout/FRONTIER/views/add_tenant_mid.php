 ...
 */
		...
		if($property_type=='VTENANT'){
			...
			$VlanIDOb=new VlanID();
            $pre_vlan = $VlanIDOb->getcount_vlanid($property_id);
            //print_r($pre_vlan);
...
//////////////////////////////////////////////
			...
			$query0 = "INSERT INTO `mdu_vetenant` (`email`,`first_name`,`last_name`,username,`password`,room_apt_no,`question_id`,property_id,`answer`,create_date,email_sent,create_user,company_name,address,city,state,postal_code,country,phone,service_profile,registration_from,vlan_id,validity_time)
						VALUES('$customer_email','$first_name','$last_name','$user_name_n','$password','$room', '$question','$property_id','$answer',NOW(),'$email_sent','$sys_user','$comp_name','$street_address','$city','$state','$postal_code','$country','$phone','$service_profile_product','MDU_PORTAL','$vlan_id','$validity_time') ON DUPLICATE KEY UPDATE 
                                    ...
                                    country = '$country',
                                    `phone` = '$phone',
                                    service_profile = '$service_profile_product',
                                    registration_from = 'MDU_PORTAL',
                                    `vlan_id` = '$vlan_id',
			...
			$response = $nf->masterAcc($user_name_n, $password, $property_id,$service_profile_product, $first_name, $last_name, $customer_email, $phone, urlencode($user_date_string),$validity_time,$country,$street_address,$state,$city,$postal_code,$question,$answer,$vlan_id);
 ...
 */
                if($property_type=='VTENANT') {
                    $post_vlan = $VlanIDOb->getcount_vlanid($property_id);
//print_r($post_vlan);
                    if ($pre_vlan['percentage'] >= $pre_vlan['notify_percentage'] && $post_vlan['percentage'] < $post_vlan['notify_percentage']) {
                       $vlan_notify_email_url = $base_portal_url.'/sheduled/vlan_notification.php?vlan='.$property_id;
                        httpGet($vlan_notify_email_url);
                    }
                }
$message_full = strtr($a, $vars);
 $message = mysql_escape_string($message_full);
  ...
  $qu = "INSERT INTO `admin_invitation_email` (`to`,`subject`,`message`,`distributor`,`user_name`,`password_re`, `create_date`)
	VALUES ('$to', '$subject_all', '$message', '$mno_id', '$new_user_name','$password', now())";
$rrr = mysql_query($qu);
...
VALUES ('$to', '$subject', '$headers', '$message', '0', 'FAILED',NOW(),'$sys_user')";
				...
				$Valid_From=$data_arr['Valid-From'];
				$Valid_Until=$data_arr['Valid-Until'];
				$queryd = "UPDATE `mdu_vetenant` SET
                                    `valid_until` = '$Valid_Until',
                                    `valid_from` = '$Valid_From'
                                    WHERE username = '$user_name_n'";
                 $db->execDB($queryd);
...
<strong>".$message_functions->showMessage('tenant_acc_creat_success','2001')."</strong></div>";
				...
				$_SESSION['msg'] = "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button>
<strong>".$message_functions->showMessage('tenant_acc_email_duplicate')."</strong></div>";
...
?>
         ...
         <h2>Add Tenant<img data-toggle="tooltip" title="In case a tenant has issues during self-registration, you can assist the tenant by establishing an account. As the tenant logs-on for the first time, he/she can sign in using the temporary password you have assigned. The tenant should also be prompted to change his/her password as soon as they login." src="layout/FRONTIER/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;"></h2>
         </div>
...
?>
	...
	<fieldset>
		...
		<div id="response_d1"></div>
					...
					<label class="control-label" for="radiobtns">First Name<span class="required-mark" style="color:#FF0000">*</span></label>
...
//																			return true;
					...
					<label class="control-label" for="radiobtns">Last Name<span class="required-mark" style="color:#FF0000">*</span></label>
...
//																			return true;
					...
					<label class="control-label" for="radiobtns">Password<span class="required-mark" style="color:#FF0000">*</span></label>
...
//	$('#password').attr('type', 'text');
  ...
  input.attr("type", "password");
	   ...
	   <div class="control-group">
	   <label class="control-label" for="radiobtns">Re-enter Password<span class="required-mark" style="color:#FF0000">*</span></label>
...
if (input.attr("type") == "password") {
  ...
  input.attr("type", "password");
					...
					<!-- /control-group -->	
						...
						<label class="control-label" for="radiobtns">Street Address<span class="required-mark" style="color:#FF0000">*</span></label>
					...
					<label class="control-label" for="radiobtns">Email Address<span class="required-mark" style="color:#FF0000">*</span></label>
				...
				<label class="control-label" for="radiobtns">Group / Realm<span class="required-mark" style="color:#FF0000">*</span></label>
...
/* $('#customer_form').formValidation('revalidateField', 'email'); */
		  ...
		  <label class="control-label" for="radiobtns">Country </sup></label>
...
// populateCountries("country");
				...
				<label class="control-label" for="radiobtns">State/Province<span class="required-mark" style="color:#FF0000">*</span></label>
...
$("#postal_code").attr('maxlength','10');
				...
				<label class="control-label" for="radiobtns">Postal Code<span class="required-mark" style="color:#FF0000">*</span></label>
...
$('#postal_code').bind("cut copy paste",function(e) {
				...
				<div class="control-group">
							<label class="control-label" for="radiobtns">City<span class="required-mark" style="color:#FF0000">*</span></label>
...
$('#city').bind("cut copy paste",function(e) {
			...
			<label class="control-label" for="radiobtns">Secret Question<span class="required-mark" style="color:#FF0000">*</span></label>
			...
			<label class="control-label" for="radiobtns">Answer<span class="required-mark" style="color:#FF0000">*</span></label>
