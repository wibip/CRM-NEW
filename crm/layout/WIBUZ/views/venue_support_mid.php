...
//print_r($bulk_profile_data_pri);        
				...
				foreach ($bulk_profile_datasession as $key => $value) {
					...
					if(in_array($bb, $testarr)){
						...
						$newarray=array("Mac"=>$value['Mac'],
                            ...
                            "Product"=>$newarray1[$value['User-Name']],
		                    "TYPE"=> $value['TYPE']);
		...
		//print_r($res_arrnew);
		$resultArray = uniqueAssocArray($res_arrnew, 'User-Name');
		//print_r($resultArray);
		$new1=array();
		$new2=array();
		//$test=array();
...
$ale->provate_module = $private_module;
...
// delete device**********************
	...
	if(isset($_GET['change_package'])){
		if($_GET['token']==$_SESSION['FORM_SECRET']){
			$m_device_mac=$_GET['m_device_mac'];
			$m_device_realm=$_GET['m_device_realm'];
			$m_session_token=$_GET['m_session_token'];
			$package=$_GET['change_package'];
			$token=$_GET['token'];

			$product1='QoS-Profile-10x64K';
            ...
            //$network_user=$rm_device_mac.'@'.$rm_device_realm;
			$response=$nf->updateAccount($m_device_mac,$m_device_realm,$productnew);
			...
			if($newarray['status_code']=='200'){
				...
				if (strlen($m_session_token)>0) {
					$responsenew=$ale->Refreshsessions($m_device_mac,$m_device_realm,$m_session_token);
					$newarrayy= json_decode($responsenew,true);
					if($newarrayy['status_code']=='200'){
							$rm_device_mac_msg = 'Package update successful';//$message_functions->showMessage('device_demove_success');
							$_SESSION[add_msg1] = "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'></button><strong>".$rm_device_mac_msg."</strong></div>";

					}
					else{
					    $rm_device_mac_msg = 'Session Refresh Faild';//$message_functions->showMessage('session_remove_fail');
					    $_SESSION[add_msg3] = "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'></button><strong>".$rm_device_mac_msg."</strong></div>";
					...
					$rm_device_mac_msg = 'Package update successful';
					$_SESSION[add_msg1] = "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'></button><strong>".$rm_device_mac_msg."</strong></div>";
				}
                ...
                $rm_device_mac_msg = 'Package update failed';//$message_functions->showMessage('device_remove_failed','2009');
								...
								<form action="?t=1" id="mac_search" class="form-horizontal" method="post">
									...
									<table id="device_sessions" class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
										...
										?>
											...
											<th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="4">Limit Package</th>
										</tr>
										...
										if(isset($_POST['search_mac_sessions']) || isset(
											...
											echo '<td>';
												if(strlen($device_mac)>0) {
													echo '<a id="' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '"  class="btn btn-small btn-danger">

														<i class="btn-icon-only icon-mobile-phone"></i>&nbsp;' . $textspeed . '</a>
														<script type="text/javascript">
                                                            $(document).ready(function() {
                                                            $(\'#' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '\').easyconfirm({locale: {
                                                                    title: \'Change Package\',
                                                                    text: \'Are you sure you want to Change this Package?&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\',
                                                                    button: [\'Cancel\',\' Confirm\'],
                                                                    closeText: \'close\'
                                                                     }});
                                                                $(\'#' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '\').click(function() {
                                                                    window.location = "?token=' . $secret . '&t=1&change_package='.$productn.'&m_device_realm='.$sh_realm.'&m_session_token='.$ses_id.'&m_device_mac=' . $mac . '&' . $next_search_pram . '=' . $mac . '"
                                                                });
                                                                });
                                                        </script>
														';
													 ...
													 echo '<a disabled   id="' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '"  class="btn btn-small btn-danger">

														<i class="btn-icon-only icon-mobile-phone"></i>&nbsp;Limit Speed</a>'; 
											 ...
											 //echo "<td><a href='#' disabled> REMOVE</a></td>";
												...
												if(strlen($device_mac)>0) {
													echo '<a id="' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '"  class="btn btn-small btn-danger">
														...
														<i class="btn-icon-only icon-mobile-phone"></i>&nbsp'.$textspeed.'</a>
														<script type="text/javascript">
                                                            $(document).ready(function() {
                                                            $(\'#' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '\').easyconfirm({locale: {
                                                                    title: \'Change Package\',
                                                                ...
                                                                $(\'#' . $session_row_count . 'CHANGE_PACKAGE_' . $mac . '\').click(function() {
                                                                    window.location = "?token=' . $secret . '&t=1&change_package='.$productn.'&m_device_realm='.$sh_realm.'&m_session_token='.$ses_id.'&m_device_mac=' . $mac . '&' . $next_search_pram . '=' . $next_search_value . '"
                                                                });
                                                                });
                                                        </script>
														';
