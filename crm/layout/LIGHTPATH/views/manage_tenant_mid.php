...
$realm=$db->getValueAsf("SELECT property_id as f FROM mdu_distributor_organizations WHERE distributor_code='$user_distributor'");


$wag_ap_name=$db->getValueAsf("SELECT `settings_value` AS f FROM `exp_settings` WHERE `settings_code`='wag_ap_name' LIMIT 1");

$ap_q1="SELECT `ap_controller`
FROM `exp_mno_distributor`
WHERE `distributor_code`='$user_distributor'
LIMIT 1";

$query_results_ap=mysql_query($ap_q1);
while($row=mysql_fetch_array($query_results_ap)){
...
if($wag_ap_name!='NO_PROFILE'){
    ...
    $wag_obj->setRealm($realm);
    $wag_obj->setDistributor($distributor);
...
if(isset($_POST['device_pass_submit'])){ 
  ...
  $cust_uname=$db->getValueAsf($cust_name_q);;
	...
	// if ($dpsk_enable) {
	// 	$new_user_nam = $cust_uname;
	// 	$querynew="SELECT u.`network_id`,d.`zone_id` FROM `exp_locations_ssid_vtenant` u JOIN `exp_mno_distributor` d ON u.`distributor`=d.`distributor_code` WHERE  d.`distributor_code` = '$user_distributor'";
	// 			$ex3 = mysql_query($querynew);
	// 			$data_zone=mysql_fetch_assoc($ex3);
	// 			$zoneId=$data_zone['zone_id'];
	// 			$wlanId=$data_zone['network_id'];

	// 			$responce_ssid=$wag_obj->deleteDPSKClient($zoneId,$wlanId,$new_user_nam,$passcode_key);
	// 			$responce_arr=json_decode($responce_ssid,true);
	// 			$status_code=$responce_arr['status_code'];
	// 			if ($status_code==200) {
	// 				$responce_ssid=$wag_obj->addDPSKClient($zoneId,$wlanId,'',$passcode_key,$new_user_nam);
					
	// 				   $responce_arr=json_decode($responce_ssid,true);
	// 				   $status=$responce_arr['status_code'];
	// 			}
	// 			else{
	// 				$responce_ssid=$wag_obj->addDPSKClient($zoneId,$wlanId,'',$passcode_key,$new_user_nam);
	// 				   $responce_arr=json_decode($responce_ssid,true);
	// 					$status=$responce_arr['status_code'];
	// 			}
	// 		   // $responce_ssid=$wag_obj->addDPSKClient($zoneId,$wlanId,$mac_address,$div_passcode_key,,$nick_name);
	// 		   // $responce_arr=json_decode($responce_ssid,true);
	// 		   // $status=$responce_arr['status_code'];
	// 		   //$status=200;
	// 		   }
...
WHERE uo.user_name='$user_name' LIMIT 1");
  ...
  $cust_name=$db->getValueAsf($cust_name_q);;
   ...
   //delete sub acc**************************
        ...
        $sub_del_respo = array();
    $querynew="SELECT u.`network_id`,d.`zone_id` FROM `exp_locations_ssid_vtenant` u JOIN `exp_mno_distributor` d ON u.`distributor`=d.`distributor_code` WHERE  d.`distributor_code` = '$user_distributor' AND u.`wlan_name` LIKE '%vtenant%'";
        $ex3 = mysql_query($querynew);
        $data_zone=mysql_fetch_assoc($ex3);
        $zoneId=$data_zone['zone_id'];
        $wlanId=$data_zone['network_id'];
        ...
        while ($devRow = mysql_fetch_assoc($sub_ass_r)){
            ...
            $responce_ssid=$wag_obj->deleteDPSKClient($zoneId,$wlanId,$devRow['net_user'],$devRow['dpsk_key']);
            $responce_arr=json_decode($responce_ssid,true);
            //$status_code=$responce_arr['status_code'];
        ...
        foreach ($not_del_sub_acc as $key => $net_user){
          ...
          if($status_code == 200 || $status_code == 404){
            $responce_ssid=$wag_obj->deleteDPSKClient($zoneId,$wlanId,$net_user,$devRow['dpsk_key']);
            $responce_arr=json_decode($responce_ssid,true);
...
else if(isset($_POST['passcode_submit'])){ 
  if($_SESSION['FORM_SECRET'] == $_POST['form_secret2']){//key validation 
  $passcode_key = trim($_POST["passcode_key"]);
   $vt_pro = $package_functions->getOptions('VTENANT_NETWORK_PROFILE',$system_package);
    ...
    $manage_customer_enable=1;
    $property_id=$_POST['property_idd'];
    $cust_uname = $_POST["edit_customer_un"];
    $mg_customer_id = $_POST['mg_customer_id'];
...
WHERE uo.user_name='$user_name' LIMIT 1");
   ...
   $qd_get = "SELECT mac_address,description,dpsk_key as d2 FROM mdu_customer_devices WHERE id = '$rm_device_id' LIMIT 1";
  $get_dev_name=mysql_query($qd_get);
  while($rowe=mysql_fetch_array($get_dev_name)){
     $mac_address=$rowe['mac_address'];
     $discription1=$rowe['d2'];
     $div_passcode_key=$rowe['dpsk_key'];
  }
  ...
  $cus_property_type= $db->getValueAsf("SELECT `property_type` AS f FROM `mdu_organizations` WHERE `property_id`='$cust_property_id'");
   ...
   $sessionstatus=json_decode($del_ses_response,true);
    ...
    //     $status_code='';
    //     $responce_ssid=$wag_obj->deleteDPSKClient($zoneId,$wlanId,$mac_address,$div_passcode_key);
    //     $responce_arr=json_decode($responce_ssid,true);
    //     $status_code=$responce_arr['status_code'];
    // }
...
if (strlen($network_profile) > 0) {
    ...
    $nf = null;
          ...
          $record_found=1;
                ...
                $div_passcode_key = $_POST['div_passcode_key'];
...
//         $responce_ssid=$wag_obj->addDPSKClient($zoneId,$wlanId,$mac_address,$div_passcode_key);
//         $responce_arr=json_decode($responce_ssid,true);
//         $status=$responce_arr['status_code'];
//       }
//     else{
    ...
    //////////////////////////////////////////////////////////
        ...
        if(mysql_num_rows($result_0) >= 1){
            $query1 = "UPDATE `mdu_customer_devices` SET
                ...
                `dpsk_key` = '$div_passcode_key'
                create_user = '$user_name'
                WHERE mac_address = '$short_desc'";
        }else {
            $query1 = "REPLACE INTO `mdu_customer_devices` (`customer_id`,`email_address`,`group`,`nick_name`,user_name,`description`,`mac_address`,`dpsk_key`,create_user,`create_date`) VALUES
        ('$mg_customer_id','$cust_email','$property_id','$nick_name','$username','$original_mac','$mac_address','$div_passcode_key','$user_name',NOW())";
        }
       ...
       <strong>Device has been successfully added</strong></div><button style=\"display:none;\" type=\"button\" class=\"btn btn-info btn-lg\" data-toggle=\"modal\" data-target=\"#myModal\"id=\"auto_click\"></button>
            ...
            <div class=\"modal fade\" id=\"myModal\" role=\"dialog\">
              <div class=\"modal-dialog\">
                ...
                <div class=\"modal-content\">
                  <div class=\"modal-body\">
                    <img style=\"width: 100%;\" src=\"img/Onboarding WiFi popup.png\">
                  </div>
                  <div style=\"border-top: none; background-color: #FFFFFF\" class=\"modal-footer\">
                    <button type=\"button\" class=\"btn btn-primary\" data-dismiss=\"modal\">Close</button>
                  </div>
...
//   }else{
//     $msg_edit= "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button><strong>".$message_functions->showMessage('vtenant_dpsk_failed','2001')."</strong></div>";
// }
}else{
        ...
        <div align="center" style="vertical-align:top;">
         ...
         <br>
             ...
             <div class="widget">
               ...
               <form id="edit_customer_form" name="edit_customer_form?tab=account&token=<?php echo $_SESSION['FORM_SECRET'];?>&search_id=<?php echo $search_id;?>&mg_email=<?php echo $mg_email; ?>&mg_customer_id=<?php echo $mg_customer_id;?>" action="manage_tenant.php" method="post"  class="form-horizontal" enctype="multipart/form-data" onchange="customer_submitenable()">
...
$('#edit_customer_form').data('bootstrapValidator').updateStatus('phone', 'NOT_VALIDATED').validateField('phone');
         ...
         <?php
          ...
          <div class="widget-content">
               ...
               <form id="passcode_form" name="passcode_form" action="manage_tenant.php?tab=account" method="post"  class="form-horizontal" style="margin: 0 0 -60px;" enctype="multipart/form-data">
                         
                           <?php
                         echo '<input type="hidden" name="form_secret2" id="form_secret2" value="'.$_SESSION['FORM_SECRET'].'" /><input type="hidden" name="search_id" id="search_id" value="'.$search_id.'" /><input type="hidden" name="mg_customer_id" id="mg_customer_id" value="'.$mg_customer_id.'" />';
                         if ($passcode_new_key) {
                           $mg_passcode_key=$passcode_new_key;
                         }
                                 
                         ?>
                         
                           <fieldset class="se_main_field_new">
         
                           <div class="se_field_new">
                             <div id="response_d5"></div>

                             <div class="control-group">
         
                                     <div class="controls form-group">
                                       <div class="columns" style="margin-bottom: 5px">
                                          <h3><strong>Add New Device Passcode Key</strong></h3>
                                       </div>
                                     </div>

                             </div>
                           
                                 <input class="span4 form-control" id="property_idd" name="property_idd" type="hidden" style="width:70%;"  value="<?php echo $mg_property_id; ?>" >
                                 <input type="hidden" name="edit_customer_un" value="<?php echo $mg_username; ?>">  

                                   <div class="control-group">
         
                                       <div class="controls form-group">

                                       <label class="" for="radiobtns">Device Passcode<sup><font color="#FF0000" >*</font></sup></label>
                                        
                                         <input style="width:70%;" class="span4 form-control pass_msg" id="passcode_key" placeholder="******" name="passcode_key" type="password"  value="<?php echo $mg_passcode_key; ?>" >
              ...
              if (input.attr("type") == "password") {
                ...
                input.attr("type", "password");
                                   ...
                                   <div class="form-actions" >
                                     ...
                                     <button type="submit" name="passcode_submit" id="passcode_submit" class="btn btn-primary">Update</button>&nbsp; <!-- <strong><font color="#FF0000">*</font><font size="-2"> Required</font></strong> -->
...
<br>
                         <form id="device_pass_form" name="device_pass_form" action="manage_tenant.php?tab=account" method="post"  class="form-horizontal" enctype="multipart/form-data" autocomplete="off">
								...
								?>
									...
									<fieldset>
                                     ...
                                     <label class="" for="radiobtns">Device Password</label>
          ...
          <p>Once a device has been registered using the 
"Register WiFi" you must use this key to login to the "Internet WiFi".
Minimum 8 characters must include letters, numbers and special characters.
Ex. YOurS3cuRe!</p>
														</div>
														<!-- /controls -->
													</div>
...
<td><?php echo macDisplay($mac_relm[0]); ?></td>
...
<small class=" help-block"  id="msg_mac<?php echo $di; ?>"   style="display: none;"></small>
						...
						<button type="submit" style="border-radius: 0px;padding-bottom: 4px !important; padding-top: 4px !important; min-width: 0 ;" href="javascript:void();" id="device_submit<?php echo $di; ?>" name="device_submit" class="btn btn-primary">
																Add</button></td>
                        <script type="text/javascript">
                        $(document).ready(function() {
...
<script type="text/javascript">
    ...
    $('#device_form').bootstrapValidator({
