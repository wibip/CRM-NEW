...
<div class="navbar navbar-fixed-top"  <?php if($top_menu=='bottom'){ ?> style="display: none;" <?php } ?>>
  <div class="navbar-inner">
    ...
    $log_img = '<img class="logo_img" src="'.$logo_image_url.'" border="0">';
    //$log_img = '<div class="logo_image"></div>';
...
<li class="dropdown" style="<?php echo $li_style; ?>"><a style="<?php echo $a_style; ?>" href="#" class="dropdown-toggle" data-toggle="dropdown">
	...
	<a style="<?php echo $a_style; ?>" href="#" class="dropdown-toggle" data-toggle="dropdown">
		Support: <strong><meta name="format-detection" content="telephone=no"><?php echo $support_number; ?></strong>
	</a>
</li>	
...
<div class="subnavbar" id="subnavbar_id">
  ...
  <div class="subnavbar-inner">
    <div class="container">
      <ul class="mainnav" style="direction: rtl;">
...
if($menutype=="SUB_MENU"){
	...
	if($top_menu=="bottom"){
				echo '<li class="no_arrow" style="line-height:100px; margin-right: 10px"><a href="intro" style="line-height: 100px !important;text-align:left !important">'.$log_img.'</a></li>';
...
/* 	if($user_type=="MNO" || $user_type=="ADMIN"){
	...
	foreach ($main_mod_array as $keym => $valuem){
		...
		if($main_menu_clickble=="NO"){
			...
			echo '<ul id="sami_'.$keym.'a" style="display: none;">';
			$link_main_m_multy = '';
...
<script type="text/javascript">
	...
	if($(window).width()<520){
		...
		$('.nav-tabs:not(.dropdown_tabs):not(.zg-ul-select) li:first-child').removeClass('active');

		$('.tab-content:not(.home-inner-tab):not(#myTabContentQuick)>.tab-pane').removeClass('active');
	}
