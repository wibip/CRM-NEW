...
$q = "SELECT * FROM `admin_product_controls_custom` WHERE `product_id` = '$system_package'";
$result=$db->Select1DB($q);
//$resultarr=json_decode($result,true);
$custom_data = json_decode($result['settings'],true);
$parent_system_package=$custom_data['general']['PARENT_PRODUCT_CODE']['options'];
...
//echo $query_set;
 ...
 if(isset($_POST['submit_group'])){//2
	...
	$query_update = "REPLACE INTO `exp_texts` (`text_code`, `text_details`, `user_type`, `distributor`, `create_date`, `updated_by`)
		...
		$query_temp = "INSERT INTO `mdu_vetenant_comunicate` (`email_id`, `property_id`, `mno_id`, `system_package`, `title`, `message`, `create_user`, `create_date`, `last_update`) 
		VALUES ('$email_id', '$property_id', '$mno_id', '$system_package', '$title', '$group_message', '$user_name', now(),  now())";
		$ex_query_temp = mysql_query($query_temp); 

		    $exec_cmd = 'php -f'.dirname(__FILE__).'/../../../sheduled/comunicate_handler.php '.$email_id.' > /dev/null 2>&1 & echo $!; ';
...
<div class="main">
							...
							<div class="widget-content" id="now">
								...
								<div class="tabbable">
									<ul class="nav nav-tabs newTabs">
										<li <?php if($active_tab == "individual_message"){echo" class=\"active\"";}?>><a href="#de_acc" data-toggle="tab">Individual Message</a></li>
									    <li <?php if($active_tab == "group_message"){echo" class=\"active\"";}?>><a href="#de_session" data-toggle="tab">Group Message</a></li>
									</ul>
									<br>
									...
									<div class="tab-content">
										...
										<?php echo $msg_val; ?>
                                            <!-- ******************* tenant details ******************* -->
										<div class="tab-pane <?php if($active_tab=="individual_message"){echo"active";}?>" id="de_acc">
  ...
  <form id="edit-profile" class="form-horizontal" method="POST">
								  ...
								  if (strlen($text_ind)<1) {
								  	$text_ind = $db->textVal('MDU_IND_MAIL',$parent_system_package);
								  }
									  ...
									  <fieldset>
													  ...
													  <div class="control-group">
														  ...
														  <div class="controls">
														  	 ...
														  	 <label class="" for="radiobtns">Search Residents <img data-toggle="tooltip" title=" Note: You can search using First Name, Last Name or Email Address." src="layout/ARRIS/img/help.png" style="width: 20px;margin-bottom: 6px;cursor: pointer;"> </label>
  ...
  FROM `mdu_vetenant` WHERE customer_id IN ($view_customer_list) ORDER BY first_name ASC";
								  ...
								  while($row=mysql_fetch_array($query_results)){
									  ...
									  echo '<a id="C2MAIL_'.$customer_id.'"  class="btn btn-small btn-info td_btn_last">Create Message</a><script type="text/javascript">
  //$(document).ready(function() {
  $(\'#C2MAIL_'.$customer_id.'\').easyconfirm({locale: {
		  title: \'Create Customer Message ['.$first_name.' '.$last_name.']\',
...
<div class="tab-pane <?php if($active_tab == 'group_message') echo 'active';?>" id="de_session">
							
							

<h1 class="head"><span>
    Group Messaging Editor <img data-toggle="tooltip" title="You can use this feature to communicate via email directly with all tenants at a property. The message goes to the tenant's email on record." src="layout/ARRIS/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;"></span>
</h1>
								...
								<form id="edit-profile" class="form-horizontal" method="POST" action="?tab=group_message">
								...
								?>
									...
									<fieldset>
										...
										<!-- <div class="control-group">
												...
												<?php
												 $group_msg = $db->textVal('MDU_GROUP_MAIL',$distributercode);
												 if (strlen($group_msg)<1) {
												  	$group_msg = $db->textVal('MDU_GROUP_MAIL',$parent_system_package);
												  }	
												  ?>
															...
															<div class="">
																...
																<textarea width="100%;" id="group_message" name="group_message" class="group_message"><?php echo $group_msg; ?></textarea>
