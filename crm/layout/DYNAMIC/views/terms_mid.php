...
//echo $edit_pro_id;
            ...
            $('#appLink').attr("href", "plugins/theme_create/mduThemeView.php?reg_type=" + $('#reg_type').val() + "&page=<?php echo $script; ?>&property=" + $('#property_id_cre').val() + "&template_name=" + $('#template_code').val() + "&dist=<?php echo $user_distributor; ?>&theme_id_ori=<?php echo $edit_theme_id; ?>&property=" + $('#property_id_cre').val() + "&theme_id=" + $('#theme_id').val() + "&enc=" + $('#enc').val()+"&chatbot="+chatBot);
        }
...
<script type="text/javascript">
	...
	<?php }?>
												...
												//echo $pieces_mvno[0];
                                                                                                    ...
                                                                                                    /*$package_functions->isDynamic($system_package);
                                                                                                    $ar = explode('-',$system_package);*/
                                                                                                    if ($package_functions->isDynamic($system_package)) {
                                                                                                        $rowt = $db->select1DB("SELECT default_campaign_id FROM exp_mno WHERE mno_id = '$user_distributor'");
                                                                                                        //$rowt=mysql_fetch_array($query_get_default_ad);
                                                                                                        $default_ad_id_new=$rowt['default_campaign_id'];
                                                                                                    }
                                                                                                    if ($default_ad_id_new==0) {
                                                                                                        $default_ad_id_new=$package_functions->getOptions("DEFAULT_AD_ID", $pieces_mvno[0]);
                                                                                                    }
