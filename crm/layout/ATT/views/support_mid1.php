		...
		<div class="container">
			<div class="row">
				<div class="span12">
					...
					<p><b>Step 1.</b> IDENTIFY ACCOUNT NUMBER OR PROPERTY ID OR BUSINESS ID: Ask the customer for their Account Number or Property ID or Business ID. The Business ID is 12 alphanumeric characters assigned to each Administrator. </p>
					
					<p><b>Step 2.</b> FIND ACCOUNT NUMBER OR PROPERTY ID OR BUSINESS ID: Enter Account Number or Property ID or Business ID in the text box below and click on "Search" button.</p>
													...
													<table class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
														<thead>
															<tr>
																...
																<th scope="col" data-tablesaw-sortable-col data-tablesaw-priority="6">ACCOUNT ADDRESS</th>
															</tr>
														</thead>
														<tbody>
														...
														echo $key_query12;
															...
															$yy = 0;
																	...
																	foreach ($query_results['data'] as $row) {
																		...
																		echo '<tr>
																		<td> '.$parent_id.' </td>
																		<td style=" border-left: 0px solid;"> <a style="align:center" href="support.php?log_other=1&key='.$encript_resetkey.'&security_token='.$token_id.'" class="btn btn-small btn-info"><i class="btn-icon-only icon-signin"></i>Sign In</a> </td><td><select data-id='.$yy.' onchange="change_val(this);">';


																		if(!isset($_POST['search_user']) || strlen($_POST['icom'])<1){
																	...
																	foreach ($query_results['data'] as $row) {
																		...
																		echo '<option data-link='.$encript_resetkey1.' data-name='.$distributor_name1.' data-address="'.trim($full_address,', ').'" value='.$icom1.'>'.$property_id1.'</option>';
																		...
																		<td > '.$is_enable.'</td>
																		<td id="ac_name'.$yy.'"> '.$first_ac_name.'</td>
																		<td id="ac_address'.$yy.'"> '.trim($first_ac_address,', ').'</td>';
														...
														<script type="text/javascript">
															function change_val(this1) {
																...
																$('#ac_name'+count).html($(this1).find(':selected').data('name'));
																$('#ac_address'+count).html($(this1).find(':selected').data('address'));
