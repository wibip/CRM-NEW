...
if($property_id!='ALL' && $property_id!=''){
    ...
    $sql="SELECT o.property_id, o.property_number,o.validity_time,ignore_on_search
            ...
            WHERE  d.`distributor_code`= u.`distributor_code` AND o.property_number = u.property_id AND d.mno_id = '$user_distributor'";
...
//exit();
        ...
        if ($property_id == "ALL" && strlen($search_word) == 0) {
            ...
            $query_set = "WHERE ( property_id in (SELECT o.property_number
                                                                FROM mdu_organizations o, mdu_distributor_organizations u, `exp_mno_distributor` d 
                                                                WHERE  d.`distributor_code`= u.`distributor_code` AND o.property_number = u.property_id AND d.mno_id = '$user_distributor')) AND (`first_name` = '$search_word' OR `last_name` = '$search_word' OR `email` = '$search_word' OR `room_apt_no` = '$search_word')";
  ...
  //  $mail_obj->mno_system_package = $system_package;
    ...
    //  $user_date_string = "usermessage=".$user_message_text."&address2=".$room_apt_no."&warning=".$aup_message_text."&country=".$country."&address=".$address."&violation=0&state=".$state."&zip=".$postal_code."&secret_question=".$question_id."&city=".$city."&secret_answer=".$answer."&company=".$comp_name;
        ...
        if($status_code == 200 || $status == 200){
            $url_base_mdu_sessions = $base_url.'/ajax/disconnect_account_sessions.php?account_place=aup_violation_submit&account_user_name='.$username.'&account_property='.$get_prperty_type_cus.'&network_name='.$vt_pro;
            httpGet($url_base_mdu_sessions);
             ...
             if($violation_count==2){
             	$service_id='AUP_VIOLATION_FINAL';
             }else{
             	$service_id='AUP_VIOLATION_1';
             }
               
               $activation_type ='Blocked';
               $reference=$username;
...
//  $search_id = $_GET['search_id'];
    ...
    $groups=explode('@',$re_username);

        $b=sizeof($groups);
        $a=$b-1;
        $property_id=$groups[$a];
    ...
    if($status_code == 200 || $status == 200){
        ...
        $url_all_mdu_sessions = $base_url.'/ajax/disconnect_account_sessions.php?account_place=aup_enable_submit&account_user_name='.$re_username.'&account_property='.$reenab_property_type.'&network_name='.$vt_pro;
       ...
       //Service Activation Report
       $service_id='AUP_RECONNECT';
       $activation_type ='Reconnected';
       $reference=$re_username;
       $db->changeFeature(new FeatureChange($service_id, $activation_type, $user_distributor, $user_type, $reference));
    }
    else{
