...
@media (min-width: 980px){
	.form-actions.save_tenant{
		width: 100% !important;
		margin-left: calc((49% - 370px)/2) !important;
	}
	.hidBr{
		display: none;
	}
...
// echo $distributer_code=$package_functions->getDistributorMONPackage($user_name);
 if($dpsk_enable){
 	$query_results_ss=mysql_query("SELECT * FROM `exp_locations_ssid_vtenant` WHERE `distributor`= '$user_distributor'");
 	while($row_s=mysql_fetch_array($query_results_ss)){
 			$ssid = $row_s['ssid'];
 			$wlan_name = $row_s['wlan_name'];
 			$networkNameLower=trim(strtolower($wlan_name));

 			if(substr($networkNameLower, 0, strlen("vtenant")) === "vtenant"){
 				$internet_ssid = $row_s['ssid'];				
							}
			elseif (substr($networkNameLower, 0, strlen("register")) === "register") {
				$onboard_ssid = $row_s['ssid'];
				$onboard_ssid_pass = $row_s['ssid_password'];
...
$(document).ready(function() {
	...
	$("#customer_submit").easyconfirm({locale: {
		title: 'New resident account',
		text: 'Are you sure you want to create this resident account?',
	...
	$("#shared_voucher_submit").easyconfirm({locale: {
		title: 'Account Voucher  ',
		text: 'Are you sure you want to create this account voucher?',
		button: ['Cancel',' Confirm'],
		closeText: 'close'
	     }});
	$("#shared_voucher_submit").click(function() {
	});
...
//Account Voucher
if(isset($_POST['shared_voucher_submit'])){ //1

	if($_SESSION['FORM_SECRET'] == $_POST['form_secret']){//key validation

		$voucher = trim($_POST["gen_voucher"]);
...
//        $data_bulk['voucher_type'] = $dpsk_voucher_type;
        ...
        if($add_voucher){
			...
			<strong>".$message_functions->showMessage('acc_voucher_creat_success','2001')."</strong></div>";
		}else{
		  $_SESSION['msg'] = "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button><strong>".$message_functions->showMessage('acc_voucher_creat_failed','2001')."</strong></div>";           
        }
...
//////////////////////////////////////////////
			...
			$query0 = "INSERT INTO `mdu_vetenant` (`email`,`first_name`,`last_name`,username,`password`,`dpsk_key`,room_apt_no,`question_id`,property_id,`answer`,create_date,email_sent,create_user,company_name,address,city,state,postal_code,country,phone,service_profile,registration_from,vlan_id,validity_time)
						VALUES('$customer_email','$first_name','$last_name','$user_name_n','$password','$passcode_key','$room', '$question','$property_id','$answer',NOW(),'$email_sent','$sys_user','$comp_name','$street_address','$city','$state','$postal_code','$country','$phone','$service_profile_product','MDU_PORTAL','$vlan_id','$validity_time') ON DUPLICATE KEY UPDATE 
							...
							<div class="widget-content" id="now">
								...
								<div class="tabbable">
									<ul class="nav nav-tabs">
										<li <?php echo isset($tab1)?'class="active"':''; ?>><a href="#create_acc" data-toggle="tab">Add Resident</a></li>
...
<?php } ?>
 ...
 <h1 class="head">Add Resident<img data-toggle="tooltip" class="tooltips" title="In case a resident has issues during the new account creation process, you can assist them by creating an account for them.
...
// });  
					...
					<div class="control-group" style="width: 100%">
						<div class="form-actions save_tenant">
							<button type="submit" name="customer_submit" id="customer_submit"  class="btn btn-primary" disabled>Save and Send Email Invitation</button> &nbsp; <br class="hidBr"><strong><font color="#FF0000">*</font><font size="1"> Required Field</font></strong>
