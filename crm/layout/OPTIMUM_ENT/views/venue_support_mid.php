...
/*			$rm_device_realm=$_GET['rm_device_realm'];
			...
			if($newarray2['status_code']=='200'){
				...
				if(true){
					$db->userLog($user_name,$script,'Add Mac',$add_mac);
	...
	if(isset($_GET['rm_device_mac'])){
		if($_GET['token']==$_SESSION['FORM_SECRET']){
			...
			if($newarray['status_code']=='200'){
				...
				$db->userLog($user_name,$script,'Delete Mac',$rm_device_mac);
...
// send log id**********************
			...
			//$rm_session_mac=$_GET['rm_session_mac'];
			 ...
			 $removed_session_array = array();
				 ...
				 if(strlen($rm_session_realm)>0){
					...
					if($newarray['status_code']=='200'){
						$db->userLog($user_name,$script,'Delete Session',$mac);
						// $delete_mac = $mac;
	...
	if(isset($_POST['search_mac_sessions']) || isset($set_search_mac_sessions) || isset($_GET['search_mac_sessions']) ){
        ...
        if (empty($newjsonvalue) && $ori_user_type!='SALES' && !isset($_GET['rm_device_mac'])) {
        	$search_mac_sessions_msg = $message_functions->showNameMessage('device_mac_search_failed',$session_search_mac,'2009');

			$_SESSION[add_msg2] = "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'></button><strong>".$search_mac_sessions_msg."</strong></div>";
						...
						<div class="widget-content">
							...
							<div class="tabbable">
								...
								<ul class="nav nav-tabs">
									...
									<?php } if(in_array('VEN_SUP_PROPERTYLOG',$features_array) || $package_features=="all"){ ?>
										<li <?php if(isset($tab5)){?>class="active" <?php }?>><a href="#property_activity_log" data-toggle="tab">Property Activity Logs</a></li>
									<?php } ?>
								</ul>
										...
										if(isset($_POST['search_mac_sessions']) || isset(
											...
											echo '<td>';
												if(strlen($device_mac)>0) {
													echo '<a id="' . $session_row_count . 'DEVICE_DELETE"  class="btn btn-small btn-danger">
													 ...
													 echo '<a disabled   id="' . $session_row_count . 'DEVICE_DELETE"  class="btn btn-small btn-danger">
...
<!-- AP LOGS END --><?php }
	...
	<!-- Property Activity Logs -->
				<div <?php if(isset($tab5)){?>class="tab-pane fade in active" <?php }else {?> class="tab-pane fade" <?php }?> id="property_activity_log">
									<div id="response_d3">
									...
									<form id="property_activity_logs_form" name="property_activity_logs_form" method="post" class="form-horizontal" action="?t=5">
											<fieldset>
												...
												<!-- 	<input type="hidden" name="api_lg" id="api_lg" value="api_lg" /> -->
													...
													<div class="control-group">
														<label class="control-label" for="limit_api">Limit</label>
														<div class="controls">
															<?php if(!isset($limit_api)){$limit_api=100;} echo '<input placeholder="100" class="span2" id="limit5" name="limit5" value="'.$limit5.'" placeholder="100" type="text">' ?>
														</div>
													</div>
										...
										<?php 
											 $row_count33 = mysql_num_rows($query_results5);
											 /* if ($row_count33<3) {
											 	$table_saw_style = 'style="margin-bottom: 60px !important;"';
											 }
											 else{
										...
										<div class="widget widget-table action-table tablesaw-minimap-mobile" <?php echo $table_saw_style; ?>>
											...
											<div class="widget-content table_response">
												...
												<table class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
													...
													while($row=mysql_fetch_array($query_results5)){
...
<!-- Property Activity Logs END --><?php } ?>
