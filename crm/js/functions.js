...
function setbring(canvas,switch_mac){
...
function doEx(method, canvas,ap_root_arr,mesh_wired_arr,mesh_wless_arr,jObj,createId,graph_code,location_id,switch_mac) {
    ...
    //hubEdge(canvas);
        ...
        case '121':
            ...
            WlessDirect(canvas,ap_root_arr,mesh_wired_arr,mesh_wless_arr,jObj,createId,graph_code,location_id);
            setbring(canvas,switch_mac);
            break;
        case '112':
...
function WlessDirect(canvas,ap_root_arr,mesh_wired_arr,mesh_wless_arr,jObj,createId,graph_code,location_id) {
    ...
    fabric.Image.fromURL(pic2, function(oImg1) {
      ...
      var text = new fabric.Text(jObj[mesh_wless_arr[0]].name, { left: 200, top: 484 ,fontSize: 10});
canvas.add(text);

    var text2 = new fabric.Text(jObj[mesh_wless_arr[0]].guest.toString(), { left: 210, top: 375 ,fontSize: 12});
canvas.add(text2);

    var text3 = new fabric.Text(jObj[mesh_wless_arr[0]].private.toString(), { left: 270, top: 375 ,fontSize: 12});
canvas.add(text3);
var status_clr,status_txt;
...
function set(data, name, createId) {
    ...
    detailsEle0 = '<a href="javascript:void(0)" onclick="hide_this(\'nodedetails\',this)" class="collapsed" aria-expanded="false" aria-controls="collapseExample"><i class="icon-plus-sign" style="font-size: 15px"></i></a><div class="" id="nodedetails" style="margin-top: 10px;display:none;max-width: 500px;margin: auto;"><div class=""><div class="">Node Details</div><div class=""><table class="table table-striped table-bordered">';
    ...
    for (key in data['apDetails']) {
        ...
        if (data['apDetails'].hasOwnProperty(key)) {
                ...
                if (data['apDetails'][key].hasOwnProperty(key1)) {
                    ...
                    if (key1 == 'type') {
                        ...
                        for (key3 in data['apDetails'][key][key1]) {
                            ...
                            size3++;
                            tab = tab + '<a class="btn btn-default show-btn collapsed" href="javascript:void(0)" onclick="hide_this(\'details00' + size + size3 + '\',this)" aria-expanded="false" aria-controls="collapseExample" > ' + data['apDetails'][key][key1][key3]['name'] + '<i class="icon-angle-right"><div></div></i></a><div class="tree_div" id="details00' + size + size3 + '" style="display: none;"><div class=""><div class=""><table class="table table-striped table-bordered">' + tableEle + '</table></div></div></div>';
            ...
            detailsEle = '<a class="collapsed" id="dddd' + size + '" href="javascript:void(0)" onclick="hide_this(\'details' + size + '\',this)" aria-expanded="false" aria-controls="collapseExample" ><i class="icon-plus-sign" style="font-size: 15px"></i></a><div  id="details' + size + '" style="margin-top: 10px;display: none"><div>';
            size++;
