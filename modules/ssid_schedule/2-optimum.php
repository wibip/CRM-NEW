...
if(isset($_SESSION['guestnet_schedule'])){
...
but on your terms.<img data-toggle="tooltip" title='You can set a broadcast schedule for each of your SSIDs. This means your SSIDs will only be visible to your visitors during the hours you have specified for each day. Ex. your hours of operations. If you select “Always ON” the SSID is visible 24/7, while “Always Off” means the SSID is turned off.' src="layout/OPTIMUM/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;">
            ...
            <div style="overflow-x:auto;" >
                ...
                <table class="table table-striped table-bordered" >
                    ...
                    if ($ori_user_type!='SALES') {
                        $delete_from_schedule_assign="DELETE FROM `exp_distributor_network_schedul_assign` WHERE `distributor`='$user_distributor' AND (`network_method`='GUEST' || `network_method` IS NULL) ";
                        ...
                        $insert_schedule_assign_guest="INSERT INTO `exp_distributor_network_schedul_assign`(`network_id`,`ssid`,`distributor`,`network_method`,`create_date`,`create_user`)
                                                                                          SELECT  `network_id`,`ssid`,`distributor`,'GUEST',NOW(),'system' FROM `exp_locations_ssid` WHERE `distributor`='$user_distributor' AND network_method<>'MDO'";

                        $db->execDB($insert_schedule_assign_guest);


                        $ssid_assign_q="SELECT ssid,ssid_broadcast,network_id FROM `exp_distributor_network_schedul_assign` a  WHERE `distributor`='$user_distributor' AND network_method='GUEST'";
    ...
    if(method_exists($wag_obj,'retrieveSchedule')){
        ...
        $ssid_q="SELECT s.network_id , s.ssid ,s.id FROM `exp_locations_ssid` s ,  `exp_mno_distributor` d
                 WHERE s.`distributor`=d.`distributor_code` AND s.`distributor`='$user_distributor' AND s.network_method<>'MDO' GROUP BY s.`ssid`";
