										   ...
										   <fieldset>
											   ...
											   <?php
												   ...
												   //$key_query1 = "SELECT DISTINCT tag_name FROM exp_mno_distributor_group_tag WHERE distributor = '$user_distributor' ORDER BY tag_name ASC";
											   		$key_query1 = "SELECT DISTINCT verification_number FROM admin_users WHERE user_distributor = '$user_distributor' AND verification_number IS NOT NULL ORDER BY verification_number ASC";
													   ...
													   <div class="input-prepend input-append">
														   ...
														   <select id="location_ssid" class="span5" name="location_ssid" required="required" onchange="valid_check(this.value,'location_ssid_validate');">
															   ...
															   echo "<option value =''> Location </option>";
																	   ...
																	   $tag_name = $row1[verification_number];
...
<br>
													   ...
													   <div class="controls uprel">
														   ...
														   <h3 style="display: inline-block;">Upload logo image<img data-toggle="tooltip" title="Acceptable Image formats are: PNG, GIF, JPEG and JPG. The maximum space the logo can occupy is 180px (width) by 100px (height). Any shape or size logo is allowed, but the system will scale the logo, keeping its original aspect ratio to fit within the above space. Maximum size is 200KB. This restriction is set to avoid the captive portal to load slow due to a large logo.Ex. If the width is more than 180px, the system will scale it into 180px and the height will be auto adjusted. If width is less than 180px and height is more than 100px, the height will be adjusted to 100px and the width will be auto adjusted." src="layout/OPTIMUM/img/help.png" style="width: 30px;margin-bottom: 6px;cursor: pointer;display: inline-block;"></h3>
...
<?php if(!in_array("welcomepage", $hide_divs_gene_arr)){ ?>
											   ...
											   <div id="isactive" class="control-group" style="<?php if($other_multi_area==1){ echo 'display: none'; } ?>">
												   ...
												   <div class="controls">
													   ...
													   <input id="is_active" name="is_active" type="checkbox" <?php if ($is_active1 == 1 && $other_multi_area!=1) { ?> checked <?php } ?>
