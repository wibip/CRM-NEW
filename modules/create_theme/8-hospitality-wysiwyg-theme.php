        ...
        <form enctype="multipart/form-data" id="theme_create_form" class="form-horizontal" method="POST" action="<?php echo $script; ?>">
            <div class="control-group">
                ...
                <div class="controls">
                    ...
                    <div style="display:none;" class="help-block error-wrapper bubble-pointer mbubble-pointer" id="shedule_name_dup">
                        <p><?php echo ucwords(__THEME_TEXT__); ?>name already exists</p>
                    </div>
                </div>
            </div>
                ...
                <div class="controls">
                    <select class="span5" id="template_name" name="template_name" required="required">
                        ...
                        if(array_key_exists('SPONSORED', $advanced) && $advanced['SPONSORED']==1){
                            array_push($pieces1,'klyde_warren_park');
                        }
                        $len1 = count($pieces1);
                        $outstr1 = "";
                        for ($i = 0; $i < $len1; $i++) {
                    ...
                    <script>
                        var init_reg_types;
                        var sponsor_reg = '<?php echo $db->getValueAsf("SELECT theme_name AS f FROM exp_theme_manager WHERE theme_code='SURVEY_PARK'"); ?>';
                        function template_changed(theme_id){
                            //alert(theme_id+"ch reg");
                            if( theme_id == 'klyde_warren_park'){
                                ...
                                $('#reg_type')
                                    ...
                                    .remove()
                                    .end()
                                    .append('<option id="'+sponsor_reg+'" value="SURVEY_PARK">'+sponsor_reg+'</option>');
                                ...
                                $('#reg_type')
                                    ...
                                    .remove()
                                    .end()
                                    .append(init_reg_types);

                                $('#sponsor_fields').hide();
                            }
                        }
                        $(function () {
                            init_reg_types = $( '#reg_type' ).children().clone();
                            $('#template_name').on('change', function() {
                                template_changed(this.value);
                            });
                            template_changed($('#template_name').val());
                        })
                    </script>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <select id="reg_type" class="span5" name="reg_type" required="required">
                        ...
                        $key_query = "SELECT theme_code, theme_name,feature_name FROM exp_theme_manager WHERE theme_type = 'NEW' AND is_enable='1' AND theme_code IN ($outstr)";
                    ...
                    if (in_array("AUTH_DISTRIBUTOR_PASSCODE", $pieces)) {
                    ?>
            ...
            <div id="sponsor_fields" style="display: none">
                <div class="control-group">
                    ...
                    <label class="control-label" for="radiobtns">Host T&C URL
                    </label>
                    <div class="controls">
                        <input type="text" class="span5" id="host_tc_url" name="host_tc_url" value="<?php echo $edit_host_tc_url; ?>">
                        <small class="help-block" style="display: none;" id="host_tc_url_validate"><p>This is a required field</p></small>
                        <small class="help-block" style="display: none;" id="host_tc_url_invalid"><p>Invalid URL</p></small>
                        <?php if ($modify_mode != 1) { ?>

                            <script type="text/javascript">
                                $('#host_tc_url').on('keyup', function(event) {

                                    if($('#host_tc_url').val().length < 1){
                                        $('#host_tc_url_validate').show();
                                    }else{
                                        $('#host_tc_url_validate').hide();
                                    }
                ...
                <div class="control-group">
                    ...
                    <label class="control-label" for="radiobtns">Sponsor T&C URL
                    </label>
                    <div class="controls">
                        <input type="text" class="span5" id="sponsor_tc_url" name="sponsor_tc_url" value="<?php echo $edit_tc_url; ?>">
                        <small class="help-block" style="display: none;" id="tcurl_t_validate"><p>This is a required field</p></small>
                        <small class="help-block" style="display: none;" id="sponsor_tc_url_invalid"><p>Invalid URL</p></small>
                        <?php if ($modify_mode != 1) { ?>

                            <script type="text/javascript">
                                $('#sponsor_tc_url').on('keyup', function(event) {

                                    if($('#sponsor_tc_url').val().length < 1){
                                        $('#tcurl_t_validate').show();
                                    }else{
                                        $('#tcurl_t_validate').hide();
                                    }
            ...
            <div class="control-group" style="<?php if ($other_multi_area == 1) {
                                                    echo 'display: none';
                                                } ?>">
...
$('#host_tc_url').on('keyup paste', function(event) {
    if($(this).val().length > 0){
        if(!validURL($(this).val())){
            $('#host_tc_url_invalid').show();
        }else{
            $('#host_tc_url_invalid').hide();
        }
    }else{
        $('#host_tc_url_invalid').hide();
    }
...
$('#splash_url').on('keyup paste', function(event) {
    urlck();
});

$('#sponsor_tc_url').on('keyup paste', function(event) {
    if($(this).val().length > 0){
        if(!validURL($(this).val())){
            $('#sponsor_tc_url_invalid').show();
        }else{
            $('#sponsor_tc_url_invalid').hide();
        }
    }else{
        $('#sponsor_tc_url_invalid').hide();
    }
...
$('#title_t').on('keyup', function(event) {
    ...
    function themeValidate(){
        ...
        if($("#sponsor_fields").is(":visible")) {
            ...
            if ($('#sponsor_tc_url').val().length < 1) {
                if (valid) {
                    $('#sponsor_tc_url').focus();
                }
                $('#tcurl_t_validate').show();
                valid = false;
            }else{
                if(!validURL($('#sponsor_tc_url').val())){
                    $('#sponsor_tc_url_invalid').show();
                    valid = false;
                }
            }
    ...
    function setTemplateImg() {
        var url = "<?php echo $base_portal_folder; ?>/template/" + $('#template_name').val() + "/img/" + $('#reg_type').val() + "-new.jpg?v=39";
        $('#loading_ajax').show();
        $('#template_img').hide();
    ...
    $(document).ready(function() {
        ...
        $("#theme_update_cancel").easyconfirm({
            locale: {
    ...
    $('#appLink').click(function(e) {
        ...
        document.cookie = "host_tc_url=" + $('#host_tc_url').val();
        document.cookie = "sponsor_tc_url=" + $('#sponsor_tc_url').val();
