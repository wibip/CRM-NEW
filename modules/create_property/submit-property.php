...
if (isset($_POST['create_location_submit']) || isset($_POST['create_location_next']) || isset($_POST['add_location_submit']) || isset($_POST['add_location_next']) || isset($_POST['property_admin'])) {//5
        $create_location_btn_action = $_POST['btn_action'];

        if ($_SESSION['FORM_SECRET'] == $_POST['form_secret5']) {//refresh validate

            $wired = isset($_POST['wired_property'])?$_POST['wired_property']:0;

            if ($user_type != 'PROVISIONING') {
            ...
            $icomme_number = empty($_POST['icomme']) ? $_POST['icomme_pvt'] : $_POST['icomme'];
            $icomme_number = empty($icomme_number) ? $_POST['vt_icomme'] : $icomme_number;
            ...
            $dpsk_conroller = trim($_POST['dpsk_conroller']);
            $dpsk_policies = trim($_POST['dpsk_policies']);
            $network_type = $_POST['network_type'];
            }else{
                $system_package = trim($_POST['system_package']);
                ...
                $guest_wlan_count = trim($_POST['guest_wlan_count']);
                $pvt_wlan_count = trim($_POST['pvt_wlan_count']);
                $vt_wlan_count = trim($_POST['vt_wlan_count']);
            ...
            if ($user_type != 'SALES') {
                ...
                elseif (in_array('GUEST', $_POST['network_type']) && in_array('PRIVATE', $_POST['network_type']) && in_array('VT', $_POST['network_type'])) {
                ...
                $old_network_type='';
                if (in_array('GUEST', $_POST['old_network_type']) && in_array('PRIVATE', $_POST['old_network_type']) && in_array('VT', $_POST['old_network_type'])) {
                    $old_network_type = 'VT-BOTH';
                } elseif (in_array('GUEST', $_POST['old_network_type']) && in_array('PRIVATE', $_POST['old_network_type'])) {
                    $old_network_type = 'BOTH';
                } elseif (in_array('GUEST', $_POST['old_network_type']) && in_array('VT', $_POST['old_network_type'])) {
                    $old_network_type = 'VT-GUEST';
                } elseif (in_array('VT', $_POST['old_network_type']) && in_array('PRIVATE', $_POST['old_network_type'])) {
                    $old_network_type = 'VT-PRIVATE';
                } elseif (in_array('GUEST', $_POST['old_network_type'])) {
                    $old_network_type = 'GUEST';
                } elseif (in_array('PRIVATE', $_POST['old_network_type'])) {
                    $old_network_type = 'PRIVATE';
                } elseif (in_array('VT', $_POST['old_network_type'])) {
                    $old_network_type = 'VT';
                }
...
//echo '2';
                ...
                $firewall_conroller = $_POST['firewall_conroller'];
                $firewall_organizations = $_POST['firewall_organizations'];
            ...
            $dateTime_zone->setTimeZone(new DateTimeZone($mvnx_time_zone));*/
                ...
                foreach ($query_results['data'] AS $row) {
                    ...
                    //// provision provisioning_setting
                ...
                if (array_key_exists('advanced_features', $provisioning_setting)) {
                    $advanced_features_arr['network_at_a_glance'] = $provisioning_setting['advanced_features']['network_at_a_glance'];
                    $advanced_features_arr['802.2x_authentication'] = $provisioning_setting['advanced_features']['802.2x_authentication'];
                    $advanced_features_arr['top_applications'] = $provisioning_setting['advanced_features']['top_applications'];
                    //$advanced_features_arr['DPSK'] = '0';
                }
                
//print_r($advanced_features_arr);
                if ($provisioning_setting['advanced_features']['top_applications'] == 1) {
//echo '1';
                    $avcEnabled = true;
                } else {
//echo '2';
                ...
                $wag_name = $provisioning_setting['wag_name'];
                $wag_enable = $provisioning_setting['content_filter'];
                $DNS_profile = $provisioning_setting['DNS_profile'];
                $DNS_profile_control = $provisioning_setting['DNS_profile_control'];
                ...
                $location_name = $db->escapeDB(trim($_POST['location_name']));
                $location_name_s = trim($_POST['location_name']);
                ...
                $dpsk_voucher = $db->escapeDB(trim($provisioning_setting['dpsk_voucher']));
                $mno_first_name = $db->escapeDB(trim($_POST['admin_f_name']));
                $mno_last_name = $db->escapeDB(trim($_POST['admin_l_name']));
                $mvnx_full_name = $mno_first_name . ' ' . $mno_last_name;
                $mvnx_email = trim($_POST['admin_email']);
                $mvnx_verify_email = trim($_POST['verify_email']);
                $mvnx_address_1 = $db->escapeDB(trim($_POST['address']));
                $mvnx_address_2 = $db->escapeDB(trim($_POST['city']));
                $mvnx_address_3 = $db->escapeDB(trim($_POST['mno_address_3']));
                $mvnx_mobile_1 = $db->escapeDB(trim($_POST['phone_number']));
                $mvnx_mobile_2 = $db->escapeDB(trim($_POST['phone_number_2']));
                $mvnx_mobile_3 = $db->escapeDB(trim($_POST['phone_number_3']));
                $mvnx_country = $db->escapeDB(trim($_POST['country']));
                $mvnx_state = $db->escapeDB(trim($_POST['state']));
                $mvnx_zip_code = trim($_POST['zip_code']);
                $mvnx_time_zone = $_POST['time_zone'];
                $firewall_conroller = $provisioning_setting['firewall_conroller'];
                $firewall_organizations = $provisioning_setting['firewall_organizations'];
                ///need correction
                $dtz = new DateTimeZone($mvnx_time_zone);
            ...
            $dateTime_zone->setTimeZone(new DateTimeZone($mvnx_time_zone));*/
                ...
                $zone_name = $db->escapeDB(trim($_POST['location_id']));//Unique property ID

                
                $tunnel = $db->escapeDB(trim($provisioning_setting['tunnel']));

                $tunnel = $db->getValueAsf("SELECT g.tunnels AS f FROM exp_gateways g WHERE g.gateway_name='$gateway_type'");
                $tunnel = trim($tunnel, '["\"]');


                $pr_tunnel = $db->escapeDB(trim($provisioning_setting['pr_tunnel']));
                $pr_tunnel = $db->getValueAsf("SELECT g.tunnels AS f FROM exp_gateways g WHERE g.gateway_name='$pr_gateway_type'");
                $pr_tunnel = trim($pr_tunnel, '["\"]');


                $zone_dec = $db->escapeDB(trim($_POST['zone_dec']));//Description
                $realm = $db->escapeDB(trim($_POST['location_id']));//Realm


                $ap_controller = $db->escapeDB(trim($provisioning_setting['ap_controller']));
                $old_ap_controller = $db->escapeDB(trim($_POST['old_conroller']));

                $zoneid = $db->escapeDB(trim($_POST['zone']));
                $groupid = $db->escapeDB(trim($_POST['groups']));

                $sw_controller = $db->escapeDB(trim($provisioning_setting['sw_conroller']));
                ...
                //Assign Default QOS Profile
                $network_info =  json_decode($_POST['network_info'],true);

                $guest_ssidarr = $network_info['Guest'];
                $pvt_ssidarr = $network_info['Private'];
                $vt_ssidarr = $network_info['VTenant'];

                $ap_control_guest = $guest_ssidarr['data'][0]['Product'];

                $ap_control = $pvt_ssidarr['data'][0]['Product'];
                $ap_control_time = $pvt_ssidarr['data'][0]['Time'];
                $get_duration_details = $db->selectDB("SELECT duration,profile_code FROM exp_products_duration WHERE id='$ap_control_time'");
                ...
                /////////////
                /// vTenant QOS
                $qos_vt_guest_def = $vt_ssidarr['data'][0]['Product'];//$db->escapeDB(trim($_POST['vt_guest_def']));
                $qos_vt_guest_pro = $db->escapeDB(trim($_POST['vt_guest_pro']));
                $qos_vt_guest_pri = $db->escapeDB(trim($_POST['vt_guest_pri']));
                ///////////////////////////////
                $subcribers_pro = $db->escapeDB(trim($_POST['subcribers_pro']));

                $live_user_name = $_POST['user_name'];
                ...
                $property_admin_new = $_POST['property_admin'];
                $user_distributor = $_POST['mno_id'];
                }

                if ($utype == 'MNO' || $utype == 'PROVISIONING') {
            ...
            WHERE u.user_distributor = d.distributor_code AND u.user_name = '$live_user_name'";
                ...
                $new_flow_account = $_POST['flow_type'];

                ////////////////// call external api //////////////////
                $network_name = $package_functions->getOptions('NETWORK_PROFILE',$system_package);
                
                if ($new_flow_account == 'new') {
                    $network_namenew = $package_functions->getOptions('NETWORK_PROFILE_NEW',$system_package);
                    if (strlen($network_namenew)>0) {
                        $network_name = $network_namenew;
                    }
                }
                ...
                if ($automation_property == 'on' || $utype == 'PROVISIONING') {
                    $automation_enable = 1;
                }else{
                    $automation_enable = 0;
                }
                if ($user_type == 'PROVISIONING') {
                    $automation_property = 'on';
                ...
                if ($network_type == 'VT' || $network_type == 'VT-PRIVATE'|| $network_type == 'VT-GUEST' || $network_type== 'VT-BOTH' || in_array('VT', $_POST['network_type'])) {
                    $vt_wlan_count = 1;
                }else{
                    $vt_wlan_count = 0;
                }
                ...
                if ($user_type != 'PROVISIONING') {
                    ...
                    $admin_features = $provisioning_setting['admin_features'];
                }
                if(!isset($_POST['location_layout_d'])  && $account_edit==1){
                    $feature_jsonn = $db->getValueAsf("SELECT `features` AS f FROM `mno_distributor_parent` WHERE  parent_id='$parent_code'");
                }else{
                    $array1 = array();
                ...
                if(isset($_POST['location_layout_d']) && $account_edit==1){
                    $dpskacc =true;
                    foreach ($admin_features as $key => $value) {
                        if ($value == 'CLOUD_PATH_DPSK') {
                            $dpskacclayout =true;
                            $vt_product=$db->getValueAsf("SELECT product_code as f FROM exp_products WHERE product_id='$qos_vt_guest_def' LIMIT 1");
                        }
                    }

                }elseif($account_edit==1) {
                    $vt_product=$db->getValueAsf("SELECT product_code as f FROM exp_products WHERE product_id='$qos_vt_guest_def' LIMIT 1");
                }else{
                ...
                $data_arr = array(
                    'guest' => $guest_wlan_count,
                    'private' => $pvt_wlan_count,
                    'vtenant' => $vt_wlan_count,
                    'flow_type' => $new_flow_account,
                    'dpsk_enable' => $dpskacc,
                    'dpsk_layout' => $dpskacclayout,
                    'wag_name' => $wag_name,
                ...
                if ($user_type != 'PROVISIONING') {
                   ...
                   $wlan_arr = json_encode(array('guest' => $_POST['no_of_guest_ssid'],
                                'private' => $_POST['no_of_pvt_ssid'],
                                'vtenant' => $_POST['vt_wlan_count'])); 
                }
                ...
                if ($account_edit == '1') {
                    ...
                    if ($create_location_btn_action != 'add_location_next') {
                        ...
                        if($_POST['vt_product_type'] == 'Group'){
                            $other_settings['vt_product_group']=1;

                        }else{
                            $other_settings['vt_product_group']=0;
                        }
                        ...
                        if ($status_code == '200' && $zonecreate == 1) {
                            ...
                            //remove multi service area assign
                            if($other_settings['other_multi_area']==0){
                                $db->execDB("DELETE FROM exp_distributor_wlan_group_ssid WHERE distributor='$edit_distributor_code'");
                                $db->execDB("UPDATE exp_locations_ap_ssid SET group_tag='$icomme_number' WHERE distributor='$edit_distributor_code'");
                                $db->execDB("DELETE FROM exp_mno_distributor_group_tag WHERE distributor ='$edit_distributor_code' AND tag_name<>'$icomme_number'");
                                $ref_like = $edit_distributor_code.'-%';
                                $db->execDB("DELETE FROM exp_customer_vouchers WHERE reference like '$ref_like'");
                            }
                            ...
                            if(isset($_POST['location_layout_d'])){
                                if(array_key_exists('CLOUD_PATH_DPSK', $array1)){}else{}

                                $queryParent = "UPDATE mno_distributor_parent SET `dpsk_controller` = '$dpsk_conroller', `dpsk_policies`='$dpsk_policies', `flow_type` = '$new_flow_account',`features` = '$feature_jsonn', system_package='$parent_package'  WHERE parent_id='$parent_code'";
                            }else{
                                $queryParent = "UPDATE mno_distributor_parent SET  `flow_type` = '$new_flow_account',`features` = '$feature_jsonn', system_package='$parent_package'  WHERE parent_id='$parent_code'";
                            }
                    ...
                    SET
                      ...
                      `automation_enable` = '$automation_enable',
                      `firewall_controller` = '$firewall_conroller',
                      `organizations_id` = '$firewall_organizations'
                        ...
                        (`product_code`,
                         ...
                         FROM `exp_products` WHERE `product_id` ='$subcribers_pro'";
                            ...
                            if($network_type == 'VT' || $network_type == 'VT-GUEST' || $network_type == 'VT-PRIVATE' || $network_type == 'VT-BOTH'){
                                ...
                                $dis_voucher = $db->getValueAsf("SELECT `voucher_code` AS f FROM `mdu_customer_voucher` WHERE `distributor` = '$edit_distributor_code' AND `voucher_type`='SHARED' AND `status` = '1'");

                                if(empty($dis_voucher)){

                                   
                                    $voucher_code = $vtenant_model->getuniqVoucherCode($mno_id);
		...
		$data_bulk['user_distributor'] = $edit_distributor_code;
...
SELECT distributor_code,property_id,create_user,create_date,last_update,'$user_name',NOW() FROM mdu_distributor_organizations WHERE distributor_code='$edit_distributor_code'";
                            ...
                            if($network_type!='VT' && $old_network_type=='VT'){
                                $update_groups = "UPDATE exp_distributor_groups SET group_name='$icomme_number' , group_number='$icomme_number' WHERE distributor='$edit_distributor_code'";
                                $update_group = $db->execDB($update_groups);
                            }else{
                                $update_group=true;
                            }
                            ...
                            if ($update_group==true && $up01===true && $up03===true && $up04===true && $up05===true && $up06===true && $qos01===true && $qos02===true && $qos03===true) {
                                //$db->execDB("COMMIT");
                                $db->commit();
                                ...
                                if($user_type == 'PROVISIONING'){
                                    //update provisioning table row state
                                    $db->execDB("UPDATE exp_provisioning_properties SET status='4' WHERE id='$update_id'");
                                ...
                                $querys = $query01.$query03.$query04.$duration_profil_1.$duration_profil_2.$query_vt_qos_1.$query_vt_qos_2.$query_vt_qos_3;
                                $error_description = $querys.'->'.$up01.$up03.$up04.$up05.$up06.$qos01.$qos02.$qos03;

                                $create_log->saveError('2002', $success_msg, $error_description);
                            }
                        } else {
                            ...
                            $sess_msg_id = 'msg_location_update';
                            $_SESSION[msg5] = "<div class='alert alert-danger'><button type='button' class='close' data-dismiss='alert'>×</button><strong>" . $success_msg . "</strong></div>";
                            $create_log->saveError('2009', $success_msg, 'API Fail');
                        }
                    ...
                    if ($status_code == "200" && $zonecreate == 1) {//1
                       // $db->execDB("SET AUTOCOMMIT=0");
                        ...
                        $parent_product = $parent_package;//$package_functions->getOptions('MVNO_ADMIN_PRODUCT',$system_package);
...
//over dpsk pool
                    ...
                    (`qos_code`,
                     ...
                     FROM `exp_qos` WHERE `id`='$selected'";
                        ...
                        $query01 = "INSERT INTO `exp_mno_distributor` (`wired`,`other_settings`,`switch_group_id`,dns_profile,dns_profile_enable,content_filter_enable,parent_id,`gateway_type`,`private_gateway_type`,`offset_val`,`wag_profile_enable`,`wag_profile`,`property_id`,`verification_number`,`network_type`,`ap_controller`,`system_package`,`zone_id`,`tunnel_type`,`private_tunnel_type`,`unique_id`,`distributor_code`, `distributor_name`,`bussiness_type`, `distributor_type`,`category`,num_of_ssid, `mno_id`, `parent_code`,`bussiness_address1`,`bussiness_address2`,`bussiness_address3`,`country`,`state_region`,`zip`,`phone1`,`phone2`,`phone3`,theme,site_title,time_zone,`language`,`advanced_features`,`is_enable`,`create_date`,`create_user`,`sw_controller`,`groupsid`,`default_campaign_id`,`dpsk_voucher`,`automation_enable`,`firewall_controller`,`organizations_id`,`wlan_count`)
                    VALUES ('$wired','$other_settings_json','$groupid','$DNS_profile','$dns_profile_enable','$dns_profile_enable','$parent_code','$gateway_type','$pr_gateway_type','$offset_val','$wag_enable','$wag_name','$zone_name','$icomme_number','$network_type','$ap_controller','$customer_type','$zoneid','$tunnel','$pr_tunnel','$unique_id','$mvnx_id', '$location_name', '$business_type','$user_type1','$category_mvnx','$mvnx_num_ssid', '$mno_id', '$user_distributor1','$mvnx_address_1','$mvnx_address_2','$mvnx_address_3','$mvnx_country','$mvnx_state','$mvnx_zip_code','$mvnx_mobile_1','$mvnx_mobile_2','$mvnx_mobile_3','$theme','$title','$tz','en','$advanced_features','0',now(),'$live_user_name','$sw_controller', '$groupsid','0','$dpsk_voucher','$automation_enable','$firewall_conroller','$firewall_organizations','$wlan_arr')";




                        $query03 = "INSERT INTO mno_distributor_parent (account_name,parent_id, system_package, mno_id, features, dpsk_controller, dpsk_policies,flow_type,create_date, create_user) VALUES ('$parent_ac_name','$parent_code', '$parent_product','$mno_id','$feature_jsonn', '$dpsk_conroller','$dpsk_policies', '$new_flow_account', NOW(),'$user_name')";

                        if ($user_type == 'PROVISIONING') {
                            $query012 = "UPDATE mdu_distributor_organizations SET distributor_code='$mvnx_id' WHERE property_id='$vt_icomme_number'";
                            ...
                            $query04 = "UPDATE `admin_users` SET 
                         `user_name`='$parent_user_name',
                         `password`=CONCAT('*', UPPER(SHA1(UNHEX(SHA1('$parend_password'))))), 
                         `access_role`='admin', 
                         `user_type`='MVNO_ADMIN', 
                         `user_distributor`='$parent_code', 
                         `full_name`='$mvnx_full_name', 
                         `email`='$mvnx_email', 
                         `mobile`='$mvnx_mobile_1', 
                         `is_enable`='2', 
                         `create_date`=NOW(),
                         `create_user`='$user_name' WHERE `verification_number`='$parent_code'";
            ...
            VALUES ('$parent_user_name','$parent_code',CONCAT('*', UPPER(SHA1(UNHEX(SHA1('$parend_password'))))), 'admin', 'MVNO_ADMIN','$parent_code', '$mvnx_full_name', '$mvnx_email', '$mvnx_mobile_1', '2', NOW(),'$user_name')";
                        ...
                        if ($package_functions->getSectionType('NET_GUEST_LOC_UPDATE', $customer_type) == 'NO' || $user_type == 'PROVISIONING') {
            ...
            `product_code`,
             ...
             FROM `exp_products` WHERE `product_id` IN ('$ap_control_guest','$ap_control')";
                        ...
                        if ($create_location_btn_action == 'create_location_submit' || $create_location_btn_action == 'create_location_next' || $property_admin_new == 1) {
...
//                             $voucher_code="READ-FELL";

		$data_bulk=array(); 

        $data_bulk['voucher'] = $voucher_code;
        $data_bulk['voucher_type'] = 'SHARED';
//        $data_bulk['voucher_type'] = $dpsk_voucher_type;
		$data_bulk['user_distributor'] = $mvnx_id;
		$data_bulk['user_name'] =  $user_name;  
		...
		$add_voucher = $vtenant_model->add_voucher($data_bulk);
                            ...
                            $cust_vo_q = "INSERT INTO `exp_customer_vouchers`(`voucher_number`,`reference`,`voucher_type`,`type`,`redeem_count`,`voucher_status`,`create_date`,`create_user`) 
                              VALUES('$manua_passcode','$mvnx_id','DISTRIBUTOR','Manual','0','0',NOW(),'$user_name')";
                            $cust_vo = $db->execDB($cust_vo_q);
                        ...
                        if ($query_result) {
                            ...
                            if ($theme_create == 'ADVANCED' || $hospitality_feature) {
                                ...
                                if($isDynamic){
                                    ...
                                    $t_data = json_decode($theme_data,true);
                                    $portal_base_folder = trim($db->setVal('portal_base_folder', 'ADMIN'), "/");
                                    foreach ($t_data['upload_arr'] as $key => $value) {
                                        if(strlen($value['value'])>0){
                                            $exp_v = explode('.',$value['value']);
                                            $vert_path = $exp_v[0].'_'.strtolower($business_type).'.'.$exp_v[1];
                                    ...
                                    $theme_data = $db->escapeDB(json_encode($t_data));
            ...
            AND settings_code IN ('ad_waiting','ad_welcome_page'))";
                            ...
                            if(strlen($subject)==0){
                                ...
                                $link = $db->setVal("global_url", "ADMIN") . '/verification_login.php?new_signin&login='.$package_functions->getSectionType('LOGIN_SIGN',$parent_product);
                                        ...
                                        $email_content = $db->getEmailTemplateVertical('PARENT_INVITE_MAIL', $system_package, 'MNO', $business_type,$mno_id);
                            ...
                            if(strlen($a)<1){
                                $a = $db->textVal('NEW_LOCATION_MAIL', 'ADMIN');
                                        ...
                                        $email_content = $db->getEmailTemplateVertical('VENUE_NEW_LOCATION', $system_package, 'MNO',$business_type ,$mno_id);
                                    ...
                                    $support_number = $package_functions->getMessageOptions('SUPPORT_NUMBER', $system_package,$business_type);
                    ...
                    VALUES ('$password', '$to', '$subject', '$message', '$mvnx_id','$email_send_option', '$dis_user_name', now())";
                                ...
                                $cunst_var['template'] = $package_functions->getOptions('EMAIL_TEMPLATE', $system_package);
                                        ...
                                        $cunst_var['verticle'] = $business_type;
                            ...
                            $network_wlan_ex = "INSERT INTO `exp_network_realm` (`id`, `realm`, `network_realm`, `network_type`, `wLan_id`, `wLan_name`, `vLan_id`, `create_user`, `create_date`, `last_update`) VALUES (NULL, '$icomme_number', '$icomme_number', 'gues', '', '', '', '$mno_id', NOW(), NOW())";
                            ...
                            $query_data = json_encode($br).$query01.$distributor_group.$distributor_group_tag.$query03.$profile_venue.$query_vt_qos_1.$query_vt_qos_2.$query_vt_qos_3.$duration_profil_1.$duration_profil_2.$query_subcribers_qos_1.$cust_vo_q.$query02.$auto_pass_ins_q;
                            $error_description= $query_data.'->'.$ex0.$ex1.$ex2.$ex3.$ex3_1.$ex4.$ex5.$ex6.$ex7.$ex8.$ex9.$cust_vo.$ex0_1.$ex4_1.$insert_passcode;
                            //echo "failed";
                            //echo $ex0.$ex1.$ex2.$ex3.$ex3_1.$ex4.$ex5.$ex6.$ex7.$ex8.$ex9.$cust_vo.$ex0_1.$ex4_1.$insert_passcode;


                            $create_log->saveError('2009', $success_msg, $error_description);
                            if ($user_type == 'PROVISIONING') {
                                return false;
                                exit();
                            }
