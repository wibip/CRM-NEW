<div <?php if (isset($tab_provision_manage)) { ?>class="tab-pane fade in active" <?php } else { ?> class="tab-pane fade" <?php } ?> id="provision_manage">

<?php

if (isset($_SESSION['msg_provision_manage'])) {
    echo $_SESSION['msg_provision_manage'];
    unset($_SESSION['msg_provision_manage']);
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls col-lg-5 form-group">
                            <select name="activeLocationsAccount" id="activeLocationsAccount"
                                ...
                                <?php
                                $fullCountQuery = "SELECT property_details FROM exp_provisioning_properties WHERE mno_id='$user_distributor' AND `status`<>9";
                                $queryResult = $db->selectDB($fullCountQuery);
                                if ($queryResult['rowCount'] > 0) {
                                    foreach ($queryResult['data'] AS $row) {
                                        $account_details = json_decode($row['property_details'],true);
                                        echo "<option class='selectors' value='" . $account_details['account_info']['business_name'] . "'>" . $account_details['account_info']['business_name'] . "</option>";
                                    }
                                }
                                ?>
                            </select>
                                ...
                                if (table.length == 0) {
                                    ...
                                    for (let index = 0; index < table.length; index++) {
                                        ...
                                        $('#EDITACC_' + element[3]).click(function () {
                                            window.location = "?t=provision_create&token=<?php echo $secret; ?>&edit&id=" + element[3];
                                        });
