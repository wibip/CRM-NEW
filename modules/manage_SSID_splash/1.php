    ...
    <div class="widget-content table_response " style="position: relative;">
        <div style="overflow-x:auto;" >
            ...
            <table id="group_tag_table" class="table table-striped table-bordered tablesaw" data-tablesaw-mode="columntoggle" data-tablesaw-minimap>
                ...
                $query4="SELECT * FROM `exp_locations_ap` WHERE `mno`='$user_distributor'";
                ...
                foreach($results['data'] AS $rows){
                    ...
                    echo '<tr>';
                    echo '<td>';
                    echo '<input type="hidden" name="check1_update'.$i.'" class="check_update" id="check_update'.$i.'" value="no">';
                    if($guest_gateway!='AC'){
                    ...
                    <script type="text/javascript">
                        ...
                        function previewn(i){
                            ...
                            if(ssid_parameter.length>1)
                                loc = loc+"&"+ssid_parameter+"="+theme_ssid;
                            ...
                            var reg_type = $('#splash_theme_id_'+i).find(':selected').attr("data-regtype");
                            var dist = $('#splash_theme_id_'+i).find(':selected').attr("data-dist");
                            var template = $('#splash_theme_id_'+i).find(':selected').attr("data-template");

                            //window.location = "plugins/theme_create/themeView.php?page=theme&reg_type="+reg_type+"&template_name="+template+"&dist="+dist+"&theme_id="+theme_id+"&enc=&action=view";
                            window.open("plugins/theme_create/themeView.php?page=theme&reg_type="+reg_type+"&template_name="+template+"&dist="+dist+"&theme_id="+theme_id+"&enc=&action=view&title="+title, '_blank');
